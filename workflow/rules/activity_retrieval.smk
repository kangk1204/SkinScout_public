...
# 운영자 선택을 워크플로가 스스로 기록하게 한다. config가 비어 있으면 예전처럼
# dev 점수 1위를 쓰지만, 값이 있으면 그 선택과 근거가 recipe.json에 남는다.
# 이 파일이 평가·게이트의 기준 파일이 되므로(docking.daina_recipe_path), 선택과
# 출하가 갈라져 게이트가 깨지는 일을 막는다.
import shlex as _shlex

AR_PREFER_RECIPE_ARG = (
    "--prefer-recipe " + _shlex.quote(str(AR_CFG["prefer_recipe"]))
    if AR_CFG.get("prefer_recipe")
    else ""
)
AR_SELECTION_RATIONALE_ARG = (
    "--selection-rationale " + _shlex.quote(str(AR_CFG["prefer_recipe_rationale"]))
    if AR_CFG.get("prefer_recipe_rationale")
    else ""
)
...
rule activity_retrieval_dev_selection:
    ...
    shell:
        ...
        python eval/activity_retrieval_model.py select \
            ...
            --out-dir {params.outdir:q} \
            {params.allow_no_improvement} \
            {AR_PREFER_RECIPE_ARG} {AR_SELECTION_RATIONALE_ARG} > {log:q} 2>&1
        """
