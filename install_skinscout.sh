...
usage() {
    printf '%s\n' \
        ...
        "  --analog-bundle-manifest-sha256 HEX  Trusted digest for a remotely fetched manifest" \
...
if ((DRY_RUN)) && [[ ! -f "${ROOT}/scripts/bootstrap_runtime.sh" ]]; then
    ...
    [[ -n "${ANALOG_BUNDLE_MANIFEST_SHA256}" ]] && bootstrap_args+=(--analog-bundle-manifest-sha256 "${ANALOG_BUNDLE_MANIFEST_SHA256}")
