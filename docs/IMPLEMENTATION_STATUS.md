...
- 현재 운영 검증: **통과** — `validate_activity_retrieval_gate.py check-operational`이 현재 runtime index·recipe·manifest 결속을 승인했다.
- 활성 recipe: 운영 `chembl_p5_max` (게이트 decision), dev-selected `union_discount_light` — `results/eval/activity_retrieval_20260914_identity/dev_selection/recipe.json` (sha256 `1c73520543165234cbf862dd2b95df086da429d272eda74e827977e6b4e0c2fe`); 실행 경로는 게이트의 operational recipe를 적용한다.
- 런타임 검색 인덱스: `data/activity_retrieval_runtime_merged_202608` (운영 게이트가 sha256으로 결속)
- operational gate `data/manifests/activity_retrieval_operational_gate.flag` (sha256 `f538bf88177d0d4a491cc411b315d1f38ecfd4877d6afebe06841b03579c5b0a`): runtime decision `diagnostic_runtime_binding`, `claim_ready=false` — 런타임 인덱스는 동결 평가 인덱스가 아니므로 동결 평가 수치를 운영 성능 주장으로 쓸 수 없다.
- 같은 게이트의 evaluation decision: `retain_frozen_baseline`, operational recipe `chembl_p5_max`, `claim_ready=false`; claim gate `status=missing`.
- 해시로 결속된 `results/eval/activity_retrieval_20260914_identity/test_evaluation/summary.json` (sha256 `2596652e565c57099e215451b4f9f7a2e998bfe834e30a068a1288d4436d3cbf`)의 46쌍 known panel: baseline `chembl_p5_max` Top10 18/46 · Top30 31/46; selected `union_discount_light` Top10 26/46 · Top30 34/46.
- temporal 2025: baseline Top10 186/300 · Top30 216/300 · MRR 0.43048; selected Top10 222/300 · Top30 237/300 · MRR 0.56512.
...
> **데이터 재생성 현황 (F24·F25, R01·R02 후속):** 완료된 부분은 F25가 지적한
> ChEMBL evidence의 PMID `.0` 문자열 정리(F05)와, per-source
> release/license/source-manifest metadata를 포함한 runtime evidence merge·
> production index 재생성이다. 남은 부분은 평가 체인(benchmark partitions →
> retrieval index → recovery panels → dev selection → final evaluation → pocket
> leakage) 재실행과 operational gate 재바인딩이다. 그전까지 위 자동 생성 블록이
> 보여주듯 `validate_activity_retrieval_gate.py check-operational`은 저장된 gate의
> runtime index 결속이 stale이라 fail-closed로 거부하며 **기본 MODE-FAST 표적
> 분석은 차단된다**. `scripts/tests/test_operational_gate_on_disk.py`의 해당
> 시험은 이 stale 상태를 정상적으로 검출해 실패한다(재바인딩 후 통과로 복귀).
> R02 후속으로 `render_current_status.py`는 이제 공식 checker 결과를 반영해
> 실패 시 현재 검증 상태를 실패·미검증으로 표시하고, 저장된 평가 수치는 역사
> 기록으로만 구분한다. BindingDB namespace 재빌드 후에는 merge → production
> index → gate binding 순으로 다시 갱신해야 한다. CosIng parquet·leakage CSV·
> similarity bands·Stage 11 pack 재생성도 아직 남아 있다.
