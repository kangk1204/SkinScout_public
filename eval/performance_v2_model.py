...
CALIBRATION_SPLIT_STRATEGY = "sha256_ligand_target_pair_hash_target_label_set_stratified"
OOD_ROUTES = (
    "in_domain",
    "ligand_cold",
    "target_cold",
...
def validate_calibration_manifest(payload: Mapping[str, Any]) -> None:
    ...
    if split.get("strategy") != CALIBRATION_SPLIT_STRATEGY:
...
def calibration_holdout_split(
    ...
    *,
    ...
    # pandas 2.x object-string vectorized concatenation silently discarded NUL
    # separators, while pandas 3.x StringDtype preserved them. Build each key
    # with Python strings so the collision-safe identity is version-independent.
    frame["_event_key"] = [
        "\0".join((ligand_id, target_id, ontology))
        for ligand_id, target_id, ontology in zip(
            frame["ligand_id"],
            frame["target_id"],
            frame["ontology"],
            strict=True,
    ...
    conflicts = frame.groupby("_event_key", sort=False)["measured_label"].nunique()
    if (conflicts > 1).any():
        raise ContractError("the same ligand/target/ontology pair has conflicting measured labels")
    # pair_features() takes ligand + target only, so every ontology variant of a
    # pair yields the same feature vector. The split unit is the ligand-target
    # pair itself: if two ontology rows of one pair crossed fit/holdout, the
    # holdout would score a feature the fit already saw.
    frame["_pair_key"] = [
        "\0".join((ligand_id, target_id))
        for ligand_id, target_id in zip(
            frame["ligand_id"],
            frame["target_id"],
            strict=True,
    ...
    pair_table = frame.groupby("_pair_key", sort=True).agg(
        target_id=("target_id", "first"),
        pair_hash=("_pair_hash", "first"),
        label_set=(
            "measured_label",
            lambda values: "|".join(sorted({str(int(value)) for value in values})),
        ),
    )
    holdout_hashes: set[str] = set()
    strata = pair_table.groupby(["target_id", "label_set"], sort=True)
    for _, group in strata:
        unique_pairs = sorted(set(group["pair_hash"].tolist()))
    ...
    fit_pair_keys = set(fit["_pair_key"].tolist())
    holdout_pair_keys = set(holdout["_pair_key"].tolist())
    if fit_pair_keys & holdout_pair_keys:
        raise ContractError("calibration fit and holdout ligand-target pairs overlap")
    ...
    evidence = {
        "strategy": CALIBRATION_SPLIT_STRATEGY,
        "split_unit": "ligand_id+target_id (feature identity; ontology variants never split)",
        "source": "train_only",
