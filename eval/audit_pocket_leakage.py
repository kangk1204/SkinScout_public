...
# Current screenable target cluster map. The older `skinscout.target-cluster-map.v2`
# names a different universe (the evidence-only 4,595-target map), so an audit
# bound to it is not evidence about the current benchmark universe.
TARGET_CLUSTER_SCHEMA_VERSION = "skinscout.screenable-target-cluster-map.v2"
LEGACY_TARGET_CLUSTER_SCHEMA_VERSIONS = ("skinscout.target-cluster-map.v2",)
...
def _validate_target_clusters(path: Path, manifest_path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if schema_version in LEGACY_TARGET_CLUSTER_SCHEMA_VERSIONS:
        raise SystemExit(
            "target cluster manifest schema "
            f"{schema_version!r} is historical/deprecated and cannot be used as "
            f"current leakage evidence; rebuild with {TARGET_CLUSTER_SCHEMA_VERSION}"
        )
    if schema_version != TARGET_CLUSTER_SCHEMA_VERSION:
...
def require_current_audit_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    """Refuse a stored audit that is not bound to the current schemas.

    A historical result can stay on disk for the record, but it must not be
    consumed as leakage evidence about the current benchmark. This is the
    consumer-side guard: `run()` writes `evidence_status: current` only after
    validating the current target cluster schema, so a legacy artifact that
    predates that field is rejected here rather than read as a current result.
    """
    if not isinstance(payload, dict):
        raise SystemExit("pocket leakage audit must be a JSON object")
    if payload.get("schema_version") != SCHEMA_VERSION:
        raise SystemExit(f"pocket leakage audit schema must be {SCHEMA_VERSION}")
    if payload.get("evidence_status") != "current" or payload.get("current_evidence") is not True:
        raise SystemExit(
            "pocket leakage audit is historical/deprecated and cannot be consumed "
            "as current leakage evidence"
        )
    inputs = payload.get("inputs") if isinstance(payload.get("inputs"), dict) else {}
    target = inputs.get("target_clusters") if isinstance(inputs.get("target_clusters"), dict) else None
    if not isinstance(target, dict) or target.get("manifest_schema_version") != TARGET_CLUSTER_SCHEMA_VERSION:
        raise SystemExit(
            "pocket leakage audit is not bound to the current target cluster schema "
            f"({TARGET_CLUSTER_SCHEMA_VERSION})"
        )
    return payload
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        payload: dict[str, Any] = {
            **summary,
            # Marks this result as bound to the current benchmark and cluster
            # schemas. `require_current_audit_evidence` refuses to consume a
            # stored audit without this marker (for example the historical
            # 202608 result bound to `skinscout.target-cluster-map.v2`).
