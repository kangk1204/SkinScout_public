...
Axes (INSTRUCTIONS.md §14.3):
    sequence: MMseqs2 alignment identity+coverage vs training-cutoff FASTA
              → seq_id ≥ 0.30; no MMseqs result means the axis is INCOMPLETE
...
SEQUENCE_MIN_ALIGNMENT_LENGTH = 20
SEQUENCE_MIN_ALIGNMENT_COVERAGE = 0.5
MMSEQS_OUTPUT_FIELDS = ("query", "target", "pident", "alnlen", "qcov", "tcov")


def _unit_fraction(value: float, label: str, line_no: int, source: Path) -> float:
    normalized = value / 100.0 if value > 1.0 else value
    if not math.isfinite(normalized) or not 0.0 <= normalized <= 1.0:
        raise RuntimeError(
            f"MMseqs {label} is outside [0, 1]/[0, 100] at line {line_no}: {source}"
        )
    return normalized
...
def _mmseqs_easy_search(
    *,
    ...
    """Best MMseqs alignment identity among sufficiently covered hits.

    Only alignment identity and alignment coverage are used.  A hit must span
    at least ``SEQUENCE_MIN_ALIGNMENT_LENGTH`` residues and cover at least
    ``SEQUENCE_MIN_ALIGNMENT_COVERAGE`` of the shorter sequence; uncovered
    micro-alignments are not treated as homologs.  No LCS/containment score is
    mixed in: when MMseqs cannot run the axis stays incomplete.
    """
    with tempfile.TemporaryDirectory(prefix="skinscout_mmseqs_") as tmp:
        ...
        cmd = [
            ...
            str(result_tsv),
            str(work_dir),
            # The axis audits UniProt protein sequences. Without an explicit
            # type a composition like "AAAA" is ambiguous and mmseqs refuses to
            # search ("It is unclear ... --search-type"), which would turn a
            # clean axis into a failed step.
            "--search-type",
            "2",
            "--format-output",
            ",".join(MMSEQS_OUTPUT_FIELDS),
            "--threads",
            "1",
        ]
        ...
        for line_no, line in enumerate(result_tsv.read_text().splitlines(), start=1):
            if not line.strip():
                continue
            parts = line.split("\t")
            if len(parts) < len(MMSEQS_OUTPUT_FIELDS):
                raise RuntimeError(
                    f"MMseqs result has fewer than {len(MMSEQS_OUTPUT_FIELDS)} "
                    f"columns at line {line_no}: {result_tsv}"
                )
            try:
                alignment_length = int(float(parts[3]))
                identity = _unit_fraction(
                    float(parts[2]), "pident", line_no, result_tsv
                )
                query_coverage = _unit_fraction(
                    float(parts[4]), "qcov", line_no, result_tsv
                )
                target_coverage = _unit_fraction(
                    float(parts[5]), "tcov", line_no, result_tsv
                )
            except ValueError as exc:
                raise RuntimeError(
                    f"MMseqs numeric field is invalid at line {line_no}: {result_tsv}"
                ) from exc
            if alignment_length < SEQUENCE_MIN_ALIGNMENT_LENGTH:
                continue
            if max(query_coverage, target_coverage) < SEQUENCE_MIN_ALIGNMENT_COVERAGE:
                continue
...
def mmseqs_max_seq_id(target_uniprot: str,
    ...
    """Alignment identity against the training cutoff, or None if incomplete.

    ``None`` means the sequence axis cannot be scored and must be reported as
    incomplete by the caller; it is never silently replaced by another
    similarity statistic.
    """
    if not query_sequence:
        return None
    sequence = "".join(str(query_sequence).upper().split())
    if not sequence:
        return None
    fasta = _resolve_fasta(training_db)
    if fasta is not None:
        if not _read_fasta(fasta):
            return None
        search_target = fasta
    elif training_db.exists():
        search_target = training_db
    else:
        return None
    if not shutil.which("mmseqs"):
        LOG.warning(
            "leakage_check: mmseqs is not available for %s — sequence axis will "
            "be treated as INCOMPLETE, not clean",
            target_uniprot,
        )
        return None
