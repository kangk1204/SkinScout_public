...
def _canonical_smiles(smiles: str) -> tuple[str, str]:
    ...
    return Chem.MolToSmiles(Chem.RemoveHs(mol), canonical=True, isomericSmiles=True), parse_mode
...
def _make_row(
    *,
    ...
    ligand_key = "SMILES:" + canonical_smiles
...
def _dedupe(rows: list[dict[str, object]]) -> tuple[list[dict[str, object]], int]:
    ...
    for row in sorted(rows, key=lambda item: (item["evidence_date"], item["source_db"], item["evidence_id"])):
        ligand_key = "SMILES:" + _canonical_smiles(_clean(row["ligand_smiles"]))[0]
        key = (
            row["publication_key"],
            row["uniprot"],
            ligand_key,
...
def _ligand_key_series(df: pd.DataFrame) -> pd.Series:
    # Source identifiers and optional InChIKeys cannot define cross-source
    # novelty. Canonical isomeric SMILES preserves stereo and collapses CCO/OCC
    # even when just one source supplies an InChIKey. Never uppercase SMILES.
    return df["ligand_smiles"].map(
        lambda value: "SMILES:" + _canonical_smiles(_clean(value))[0]
    )
        ...
        manifest: dict[str, Any] = {
            ...
            "splits": {
                "ligand_identity_policy": {
                    ...
                    "applies_to": ["benchmark_id", "deduplication", "first_observation", "leakage_audit", "cold_flags"],
                },
                "boundaries": {
                    "train": "<=2023-12-31",
