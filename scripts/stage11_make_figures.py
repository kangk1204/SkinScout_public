...
SOURCE_BACKED_RENDERINGS = {"measured_quantity"}


def caption_claim_status(
    name: str,
    rendered_content: str,
    sources: list[dict[str, object]],
    path: Path,
) -> tuple[str, bool, bool]:
    """Classify one figure caption without weakening the claim gate.

    Artifact row/key/byte counts stay diagnostic.  ``source_backed`` is only
    returned for a rendering of measured quantities where every source records
    a finite positive value, its physical unit, and the source SHA-256.
    """
    if rendered_content not in SOURCE_BACKED_RENDERINGS:
        return "diagnostic_only", True, False
    if not sources:
        raise ValueError(
            f"source-backed caption has no sources: {name}: {path}"
        )
    for idx, source in enumerate(sources):
        if not isinstance(source, dict):
            raise ValueError(
                f"source-backed caption source must be an object: "
                f"{name}: source index {idx}: {path}"
            )
        unit = str(source.get("unit", "")).strip()
        if not unit:
            raise ValueError(
                f"source-backed caption source requires a measured unit: "
                f"{name}: source index {idx}: {path}"
            )
        ...
        if (
            ...
            raise ValueError(
                f"source-backed caption source requires a finite positive "
                f"measured value: {name}: source index {idx}: {path}"
            )
        ...
        if len(sha) != 64 or any(
            ...
            raise ValueError(
                f"source-backed caption source requires a source SHA-256: "
                f"{name}: source index {idx}: {path}"
            )
    return "source_backed", False, True
    ...
    with tempfile.TemporaryDirectory(prefix=".stage11_figures_", dir=args.out_dir.parent) as tmp:
        ...
        for fname, caption, rels in FIGURE_DEFINITIONS:
            ...
            if args.allow_placeholders:
                ...
                rendered_content = "draft_placeholder"
                entry_sources = [{"path": str(p)} for p in sources]
                ...
                rendered_content = "artifact_evidence_counts"
                entry_sources = [_source_evidence(p) for p in sources]
                status, diagnostic_only, claim_eligible = caption_claim_status(
                    fname, rendered_content, entry_sources, out_path
                )
            rendered_caption = (
                caption
                if args.allow_placeholders
