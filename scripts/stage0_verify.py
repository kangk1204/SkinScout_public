...
def chk_pdbqt_roundtrip(
    ...
    if expected_targets is not None:
        missing = sorted(expected_targets - found_targets)
        unexpected = sorted(found_targets - expected_targets)
    ok = not (bad or missing or unexpected)
    detail = (
        f"{len(pdbqts)} pdbqt; {bad} suspect headers; "
        f"missing={len(missing)} [{_preview(missing)}] "
        f"unexpected={len(unexpected)} [{_preview(unexpected)}]"
    )
...
def chk_canonical_sequences(
    ...
    if set(records_by_accession) != set(sequences):
        missing_records = sorted(set(sequences) - records_by_accession)
...
def collect_checks(
    ...
    *,
    ...
    expected_targets = _clean_target_ids(data / "human_clean") if claim_quality else None
    no_pocket_targets = (
    ...
    expected_pdbqt_targets = (
        _with_pocket_targets(data / "human_pockets") - (no_pocket_targets or set())
        if claim_quality
        else None
    )
    p2rank_check = (
        ...
        if claim_quality
        else chk_p2rank(data / "human_pockets")
    )
    pdbqt_check = (
        chk_pdbqt_roundtrip(
            data / "human_pdbqt",
        ...
        if claim_quality
        else chk_pdbqt_roundtrip(data / "human_pdbqt")
    )
    checks = [
        chk_alphafold_count(data / "alphafold_human_v4"),
        chk_cleaned_count(data / "human_clean"),
        p2rank_check,
        pdbqt_check,
        chk_mmseqs(data / "mmseqs"),
        chk_table(data / "skin_expression" / "skin_score.tsv",
                  "v3_skin_score", {"uniprot", "skin_score"}),
        chk_table(data / "cosing" / "cosing.parquet",
                  "v3_cosing", {"inci_name"}),
        chk_table(data / "drug_avoidance" / "drugs.parquet",
                  "v3_drug_avoidance", {"smiles"}),
        chk_graphml(data / "skin_efficacy_kg" / "skin_efficacy.graphml",
                    "v3_skin_efficacy_kg"),
        chk_nonempty_file(data / "skin_proteome" / "skin_proteome.tsv",
                          "v3_skin_proteome"),
    ]
    if require_activity_evidence:
        checks.extend([
            chk_parquet_schema(
                data / "chembl37" / "activity_evidence.parquet",
            ...
            chk_parquet_schema(
                data / "chembl37" / "fp_morgan2_2048.parquet",
                "chembl37_fingerprints",
                {"molecule_chembl_id", "bitvec"},
            ),
            chk_evidence_manifest(
                data / "chembl37" / "source_manifest.json",
                "chembl37_source_manifest",
                {"schema_version", "source", "input_sha256", "output_sha256"},
            ),
            chk_chembl_evidence_integrity(data / "chembl37"),
            chk_chembl_fingerprint_integrity(data / "chembl37"),
            chk_discovery_alias_integrity(repo),
            chk_parquet_schema(
                data / "gtopdb" / "evidence_v1" / "activity_evidence.parquet",
            ...
            chk_parquet_schema(
                data / "gtopdb" / "evidence_v1" / "pre_cutoff.parquet",
                "gtopdb_pre_cutoff_evidence",
                {"evidence_id", "source_db", "temporal_split"},
                min_rows=0,
            ),
            chk_parquet_schema(
                data / "gtopdb" / "evidence_v1" / "post_cutoff.parquet",
                "gtopdb_post_cutoff_evidence",
                {"evidence_id", "source_db", "temporal_split"},
                min_rows=0,
            ),
            chk_evidence_manifest(
                data / "gtopdb" / "source_manifest.json",
                "gtopdb_source_manifest",
                {"schema_version", "source", "artifacts"},
            ),
            chk_evidence_manifest(
                data / "gtopdb" / "evidence_v1" / "manifest.json",
            ...
            chk_gtopdb_evidence_integrity(data / "gtopdb"),
        ])
    if require_activity_evidence and require_bindingdb_evidence:
        checks.extend([
            chk_parquet_schema(
                data / "bindingdb" / "evidence_v1" / "activity_evidence.parquet",
            ...
            chk_evidence_manifest(
                data / "bindingdb" / "evidence_v1" / "manifest.json",
                "bindingdb_evidence_manifest",
                {"schema_version", "source", "inputs", "policy", "artifacts"},
            ),
            chk_bindingdb_evidence_integrity(data / "bindingdb"),
            chk_parquet_schema(
                data / "evidence_splits" / "post_cutoff_novel_pairs.parquet",
    ...
    if claim_quality:
        checks.extend([
            ...
            chk_mmseqs_training_cutoff(data / "mmseqs"),
            chk_table_claim_quality(
                data / "skin_expression" / "skin_score.tsv",
                "claim_skin_score",
                {"uniprot", "skin_score"},
                min_skin_score_rows,
            ),
