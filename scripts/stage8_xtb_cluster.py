...
def read_crest_manifest(path: Path) -> pd.DataFrame:
    ...
    if "selected_for_qm" in df.columns:
        selection = df["selected_for_qm"].fillna("").astype(str).str.strip().str.lower()
        invalid_selection = sorted(set(selection) - {"true", "false"})
        if invalid_selection:
            shown = ",".join(invalid_selection[:10])
            suffix = "..." if len(invalid_selection) > 10 else ""
            raise SystemExit(
                "CREST manifest column 'selected_for_qm' contains invalid values: "
                f"{shown}{suffix}"
            )
        df["selected_for_qm"] = selection == "true"
    return df
...
def main() -> None:
    ...
    if "selected_for_qm" in df.columns:
        df = df[df["selected_for_qm"]].copy()
        if df.empty:
            raise SystemExit(
                "CREST manifest has no target selected for QM refinement; "
                "every MM-GBSA delta TOTAL was nonnegative (valid results, "
                "but not selected)."
            )
    if not shutil.which("xtb"):
