...
from residue_mapping import build_mapping_record, write_mapping_record
        ...
        with tempfile.TemporaryDirectory(prefix=".stage4_prepare_", dir=args.out_dir) as tmp:
            ...
            with tmp_manifest.open("w", newline="") as fh:
                ...
                w.writerow([
                    ...
                    # 관측 구조와 정본 서열을 분리해 넘긴다. 서열 hash 와 residue
                    # mapping 이 없으면 stage6 는 결손 잔기 10 과 20 을 붙여 만든
                    # 적 없는 연속 서열을 샘플링하게 된다.
                    "observed_sequence_sha256", "canonical_sequence_sha256",
                    "residue_mapping_json", "sequence_continuous",
                ])
                for uid in target_ids:
                    ...
                    mapping_record = build_mapping_record(
                        stage_input_pdb,
                        target_id=uid,
                        source=src,
                        canonical_sequence=monomer_seq,
                    )
                    final_mapping_json = (
                        final_target_dir / f"{uid}_input_residue_mapping.json"
                    )
                    write_mapping_record(
                        stage_input_pdb,
                        mapping_record,
                        stage_target_dir / final_mapping_json.name,
                    )
                    w.writerow([
                        ...
                        mapping_record["observed_sequence_sha256"],
                        mapping_record["canonical_sequence_sha256"],
                        str(final_mapping_json),
                        "true" if mapping_record["observed_continuous"] else "false",
                    ])
                    n_written += 1
            if n_written == 0:
