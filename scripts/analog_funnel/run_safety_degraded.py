            ...
            for row in failed:
                ...
                if report.is_file():
                    try:
                        ...
                        value = payload.get("missing_models")
                        if isinstance(value, list):
                            missing = ",".join(str(v) for v in value)
                    except (OSError, json.JSONDecodeError):
                        pass
                writer.writerow([row["seed_dir"], row["candidate_id"], run_id, rc,
                                 skin, cosmetic, missing])
                handle.flush()
                print(f"[degraded] {row['candidate_id']} rc={rc} skin={skin} cosmetic={cosmetic}",
                      flush=True)
                time.sleep(3)
