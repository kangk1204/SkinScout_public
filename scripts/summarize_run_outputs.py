...
from messages_ko import ACTIONS_BY_DECISION, action_label, decision_label, reason_label
...
from run_artifact_contract import (
    COMPREHENSIVE_SCREENING_ARTIFACTS,
    CONTRACT_CURRENT,
    CONTRACT_LEGACY,
    FAST_FINAL_FUNNEL_EDGE_CURRENT,
    FAST_SCREENING_ARTIFACTS_CURRENT,
    FAST_SCREENING_ARTIFACTS_LEGACY,
    TARGET_COMPREHENSIVE_ARTIFACTS,
    TARGET_FAST_ARTIFACTS_CURRENT,
    TARGET_FAST_ARTIFACTS_LEGACY,
...
SKIN_EXPRESSION_SUPPORT_BASIS = "relative_within_build_expression_context_heuristic"
SKIN_EFFICACY_EVIDENCE_TYPE = "automatic_gene_keyword_cooccurrence_review_required"
RERANK_POLICY_COLUMNS = {
    "rerank_policy_enabled",
    "rerank_policy_status",
    "rerank_keep",
    "rerank_band_end",
}
...
# Artifact sets come from the shared contract (run_artifact_contract) so the
# writer and the goal-contract verifier cannot drift apart. The aliases keep
# the names existing tests and readers already use.
FAST_TARGET_ARTIFACTS = TARGET_FAST_ARTIFACTS_CURRENT
LEGACY_FAST_TARGET_ARTIFACTS = TARGET_FAST_ARTIFACTS_LEGACY
COMPREHENSIVE_TARGET_ARTIFACTS = TARGET_COMPREHENSIVE_ARTIFACTS
FAST_SCREENING_COUNT_ARTIFACTS = FAST_SCREENING_ARTIFACTS_CURRENT
LEGACY_FAST_SCREENING_COUNT_ARTIFACTS = FAST_SCREENING_ARTIFACTS_LEGACY
COMPREHENSIVE_SCREENING_COUNT_ARTIFACTS = COMPREHENSIVE_SCREENING_ARTIFACTS
FAST_FINAL_FUNNEL_EDGE = FAST_FINAL_FUNNEL_EDGE_CURRENT
...
def _screening_funnel_edges(
    ...
    return screening_funnel_edges(
        mode,
        CONTRACT_CURRENT if daina_structural else CONTRACT_LEGACY,
    )
...
def _validated_skin_sens(
    ...
    *,
    ...
    # F30: an applicability-limited model (outside/unknown HuSSPred domain) is
    # available evidence blocked from the PASS basis, not a missing model. Its
    # call is legitimately absent, so degraded runs must tolerate it here too.
    raw_limited = skin_sens.get("applicability_limited_models") or []
    if not isinstance(raw_limited, list):
        raise SystemExit("skin-sens applicability_limited_models must be a list")
    limited_models = [str(model).strip() for model in raw_limited if str(model).strip()]
    calls: dict[str, str] = {}
    for model in SKIN_SENS_MODELS:
        call = _text_or_none(skin_sens.get(model))
        if call not in SKIN_SENS_CALLS:
            if allow_degraded and (model in missing_models or model in limited_models):
                continue
...
def _rerank_policy_metadata(df: pd.DataFrame) -> dict[str, Any] | None:
    present = RERANK_POLICY_COLUMNS & set(df.columns)
    if not present:
        return None
    missing = sorted(RERANK_POLICY_COLUMNS - set(df.columns))
    if missing:
        raise SystemExit(f"target ranking has incomplete rerank policy metadata: {missing}")
    rows: list[tuple[bool, str, int, int]] = []
    for index, row in df.iterrows():
        raw_enabled = row["rerank_policy_enabled"]
        if isinstance(raw_enabled, str) and raw_enabled.strip().lower() in {"true", "false"}:
            enabled = raw_enabled.strip().lower() == "true"
        elif isinstance(raw_enabled, bool) or type(raw_enabled).__name__ == "bool_":
            enabled = bool(raw_enabled)
        else:
            raise SystemExit(
                "target ranking rerank_policy_enabled must be boolean at row "
                f"index {index}"
            )
        status = _text_or_none(row["rerank_policy_status"])
        if status not in {"experimental", "disabled"}:
            raise SystemExit(
                "target ranking rerank_policy_status must be experimental or disabled"
            )
        keep = _integer_at_least(
            row["rerank_keep"], "target ranking rerank_keep", minimum=0
        )
        band_end = _integer_at_least(
            row["rerank_band_end"], "target ranking rerank_band_end", minimum=0
        )
        if enabled != (status == "experimental"):
            raise SystemExit(
                "target ranking rerank policy enabled/status values are inconsistent"
            )
        if enabled != (keep < band_end):
            raise SystemExit(
                "target ranking rerank policy bounds do not match enabled flag"
            )
        rows.append((enabled, status, keep, band_end))
    unique = set(rows)
    if len(unique) != 1:
        raise SystemExit("target ranking rerank policy metadata must be identical on every row")
    enabled, status, keep, band_end = unique.pop()
    return {
        "enabled": enabled,
        "status": status,
        "keep": keep,
        "band_end": band_end,
        "claim_status": "nonclaimable" if enabled else "canonical_recipe_ranking",
    }
...
def _skin_specialized_binding_summary(target_prediction: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "skin_expression_support_threshold": SKIN_EXPRESSION_SUPPORTED_THRESHOLD,
        "skin_expression_support_basis": SKIN_EXPRESSION_SUPPORT_BASIS,
        "skin_efficacy_evidence_type": SKIN_EFFICACY_EVIDENCE_TYPE,
        "skin_efficacy_requires_review": skin_efficacy_supported,
...
def _target_summary(
    ...
    *,
    ...
    rerank_policy = _rerank_policy_metadata(df)
...
def _overall_decision(
    *,
    ...
    decision = "PASS"
    efficacy_review_required = False
    reasons: list[str] = []
    ...
    if preset in {"target-id", "report"}:
        if not isinstance(target_prediction, dict):
            ...
            rerank_policy = target_prediction.get("rerank_policy")
            if isinstance(rerank_policy, dict) and rerank_policy.get("enabled") is True:
                decision = _raise_decision(decision, "FLAG_HIGH")
                reasons.append("experimental target reranking policy is enabled")
            top_targets = target_prediction.get("top_targets")
            ...
            if not isinstance(skin_specialized_binding, dict):
                ...
                # Automatic gene-keyword co-occurrence is a literature-search
                # signal, never an approved efficacy claim. It must not block the
                # operational verdict (which describes pipeline completion); it
                # only restricts what may be claimed (handled below).
                if skin_specialized_binding.get("skin_efficacy_requires_review") is True:
                    efficacy_review_required = True
    ...
    if efficacy_review_required:
        reasons.append(
            "skin efficacy evidence is automatic literature co-occurrence and requires review"
        )
    requires_human_review = decision != "PASS"
    # messages_ko owns the canonical action enum; the Korean renderer and the
    # wire value must not drift apart again.
    recommended_action = ACTIONS_BY_DECISION[decision]
    claimable = (
        decision == "PASS"
    ...
    return {
        ...
        "claimable": claimable,
        "efficacy_claim_allowed": decision == "PASS" and not efficacy_review_required,
        "reasons": reasons,
    ...
    if skin_binding:
        ...
        lines.extend(
            [
                ...
                f"- Skin efficacy requires review: {_display_value(skin_binding.get('skin_efficacy_requires_review'))}",
                "- Skin expression support basis: relative within-build expression context; threshold 0.20 is heuristic and is not validated efficacy evidence",
                "- Skin efficacy evidence type: automatic gene-keyword co-occurrence; mechanism, direction, species, and compound efficacy require review",
