...
def main() -> int:
    ...
    parser.add_argument(
        "--target-example",
        default="tyrosinase",
        help="protein typed into the target-search screen for its screenshot",
    )
    ...
    with sync_playwright() as p:
        ...
        # 6. the reverse direction - a protein in, measured compounds out
        try:
            page.click('.nav-item[data-view="targets"]')
            page.wait_for_timeout(1000)
            _focus_view(page, "#view-targets")
            page.fill("#targets-query", args.target_example)
            page.click("#targets-run")
            # 첫 조회는 생산 인덱스(edges 1.7M행)를 읽는다. 이후 조회는 캐시된다.
            page.wait_for_selector(
                "#targets-surface:not(.is-hidden)", timeout=180000
            )
            page.wait_for_timeout(1200)
            _dismiss_overlays(page)
            _focus_view(page, "#view-targets")
            shot("wb-7-targets.png", "target search: measured compounds for a protein")
        except Exception as error:  # noqa: BLE001
            print(f"  (target search not shown: {error})", file=sys.stderr)
