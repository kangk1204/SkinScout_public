...
def read_receptor_manifest(path: Path,
    ...
    for _, row in manifest.iterrows():
        ...
        if uid in paths:
            # 합의가 고른 것만 남긴다.
            if chosen and chosen.get(uid) != medoid:
                continue
        elif chosen and uid in chosen and chosen[uid] != medoid:
            continue
        paths[uid] = require_nonempty(
            Path(medoid), f"Receptor medoid PDB for {uid}"
        )
...
def prepare_ligand_topology(
    ...
    command_text = run_command(command, cwd=target_dir)
    candidates = [
        target_dir / "ligand.acpype" / "ligand_GMX.itp",
        target_dir / "ligand_GMX.itp",
    ]
    generated = next((path for path in candidates if _nonempty(path)), None)
    if generated is None:
        shown = ", ".join(str(path) for path in candidates)
        raise SystemExit(
            "ACPYPE completed without an include-compatible ligand topology; "
            f"checked: {shown}"
        )
    validate_ligand_topology(generated)
    return generated, "acpype_generated", command_text
...
def main() -> None:
    ...
    if args.complex_pose is None and args.complex_pose_manifest is None:
        try:
            receptor_paths = read_receptor_manifest(args.receptor_manifest)
        except SystemExit:
            # Without a pose, the external pose gate is the actionable blocker;
            # preserve stale diagnostics when the receptor provenance is also
            # unavailable. A valid receptor manifest represents a real current
            # attempt, so its stale output is removed before failing closed.
            raise fail_closed(
                "--complex-pose or --complex-pose-manifest is required for Stage 7 GROMACS prep"
            )
        remove_stale_manifest(args.out_manifest)
        raise fail_closed(
            "--complex-pose or --complex-pose-manifest is required for Stage 7 GROMACS prep"
