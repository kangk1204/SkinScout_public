...
def _validate_ligand_identity_policy(
    benchmark: dict[str, Any], source: Path, *, require_promotion: bool
) -> None:
    if "splits" not in benchmark:
        policy = None
    else:
        splits = benchmark["splits"]
        if not isinstance(splits, dict):
            raise SystemExit(f"{source} splits must be a mapping")
        policy = (
            splits["ligand_identity_policy"]
            if "ligand_identity_policy" in splits
            else None
        )
    if policy is None:
        policy_is_present = (
            isinstance(benchmark.get("splits"), dict)
            and "ligand_identity_policy" in benchmark["splits"]
        )
        if policy_is_present:
            raise SystemExit(
                "activity retrieval benchmark ligand identity policy is incomplete or invalid"
            )
        if require_promotion:
            raise SystemExit(
                "activity retrieval benchmark ligand identity policy is required "
                "for performance claims"
            )
        return
    if (
        not isinstance(policy, dict)
        or policy.get("uses_source_ligand_ids") is not False
        or policy.get("uses_optional_source_inchikey") is not False
        or policy != LIGAND_IDENTITY_POLICY
    ):
        raise SystemExit(
            "activity retrieval benchmark ligand identity policy is incomplete or invalid"
        )
    versions = benchmark.get("versions")
    rdkit_version = versions.get("rdkit") if isinstance(versions, dict) else None
    if not isinstance(rdkit_version, str) or not rdkit_version.strip():
        raise SystemExit(
            "activity retrieval benchmark ligand identity policy requires versions.rdkit"
        )
...
def _cold_claim_record(payload: dict[str, Any], label: str, source: Path) -> dict[str, Any]:
    gates = payload.get("gates")
    dual = gates.get("dual_cold") if isinstance(gates, dict) else None
    if not isinstance(dual, dict):
        raise SystemExit(
            f"activity retrieval {label} is missing the dual-cold claim coverage "
            f"record: {source}"
        )
    return dual


def _validate_cold_claim_coverage(
    final: dict[str, Any],
    summary: dict[str, Any],
    source: Path,
    summary_source: Path,
) -> dict[str, Any]:
    """Fail closed when the cold-start claim is not backed by scoreable truth.

    Panel adequacy counts queries, targets and documents; it says nothing about
    whether any frozen truth target is inside the retrieval model's scorable
    universe. A large panel with zero scoreable truth still produces rank
    numbers from the tied zero-score tail, so it cannot carry a performance
    claim.
    """
    final_dual = _cold_claim_record(final, "final evaluation manifest", source)
    summary_dual = _cold_claim_record(summary, "final evaluation summary", summary_source)
    if final_dual != summary_dual:
        raise SystemExit(
            "activity retrieval final summary dual-cold claim record is inconsistent "
            "with the final evaluation manifest"
        )
    if final_dual.get("claimable") is not True:
        raise SystemExit(
            "activity retrieval dual-cold claim coverage gate did not pass: "
            "no scorable truth coverage"
        )
    if final_dual.get("passes_truth_universe_coverage") is not True:
        raise SystemExit(
            "activity retrieval dual-cold truth-universe coverage gate did not pass"
        )
    coverage = final_dual.get("coverage")
    if not isinstance(coverage, dict) or coverage.get("measured") is not True:
        raise SystemExit(
            "activity retrieval dual-cold scorable-universe coverage was not measured"
        )
    truth_pairs = coverage.get("truth_pairs")
    in_universe_pairs = coverage.get("truth_pairs_in_scorable_universe")
    if (
        isinstance(truth_pairs, bool)
        or not isinstance(truth_pairs, int)
        or truth_pairs < 1
        or isinstance(in_universe_pairs, bool)
        or not isinstance(in_universe_pairs, int)
        or not 1 <= in_universe_pairs <= truth_pairs
    ):
        raise SystemExit(
            "activity retrieval dual-cold claim requires at least one truth pair "
            "inside the scorable universe"
        )
    recorded_coverage = coverage.get("truth_pair_coverage")
    if (
        isinstance(recorded_coverage, bool)
        or not isinstance(recorded_coverage, (int, float))
        or not math.isclose(
            float(recorded_coverage),
            in_universe_pairs / truth_pairs,
            rel_tol=1e-9,
            abs_tol=1e-12,
        )
    ):
        raise SystemExit(
            "activity retrieval dual-cold truth-pair coverage is stale"
        )
    metrics = coverage.get("in_universe")
    if not isinstance(metrics, dict):
        raise SystemExit(
            "activity retrieval dual-cold in-universe metrics are missing"
        )
    for key in ("top10", "top30", "mrr"):
        value = metrics.get(key)
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise SystemExit(
                "activity retrieval dual-cold in-universe metrics must be reported "
                f"for {key}"
            )
        if not math.isfinite(float(value)) or not 0.0 <= float(value) <= 1.0:
            raise SystemExit(
                f"activity retrieval dual-cold in-universe metric is invalid: {key}"
            )
    return final_dual
...
def validate_manifest_contract(
    *,
    ...
    _validate_ligand_identity_policy(
        benchmark,
        benchmark_manifest,
    ...
    summary_record = _required_record(
        final, "outputs", "summary.json", final_evaluation_manifest
    )
    summary_path = Path(str(summary_record.get("path", "")))
    _assert_digest(
        summary_path,
        summary_record.get("sha256"),
        "final evaluation summary",
    )
    summary_payload = _load_json(
        summary_path, MANIFEST_SCHEMAS["final_evaluation_manifest"]
    )
    cold_claim_coverage: dict[str, Any] | None = None
    if require_promotion:
        cold_claim_coverage = _validate_cold_claim_coverage(
            final,
            summary_payload,
    ...
    return {
        ...
        "final_dual_cold_coverage": cold_claim_coverage,
...
def _nonempty_string_or_list(value: object, context: str) -> None:
    if isinstance(value, str):
        ...
        return
    raise SystemExit(f"{context} must be a non-empty string or list of strings")


def _validate_runtime_source_licensing(
    runtime_index: dict[str, Any], runtime_index_manifest: Path
) -> None:
    """Every actual source must carry release/license/source-manifest metadata.

    A merged runtime table can hold ChEMBL and BindingDB rows while the manifest
    records only the added tier's licence. The gate refuses that: each source in
    `source_counts` needs its own record, and the key sets must match so a count
    table cannot silently outrun the licence lineage.
    """
    source_counts = runtime_index.get("source_counts")
    if not isinstance(source_counts, dict) or not source_counts:
        raise SystemExit(
            "activity retrieval runtime index must record per-source row counts"
        )
    counts: dict[str, int] = {}
    for source, count in source_counts.items():
        if not isinstance(source, str) or not source.strip():
            raise SystemExit(
                "activity retrieval runtime source_counts keys must be non-empty strings"
            )
        if not isinstance(count, int) or isinstance(count, bool) or count < 1:
            raise SystemExit(
                f"activity retrieval runtime source_counts.{source} must be a "
                "positive integer"
            )
        counts[source.strip()] = count

    licensing = runtime_index.get("evidence_licensing")
    if not isinstance(licensing, dict):
        raise SystemExit(
            "activity retrieval runtime index must record evidence_licensing"
        )
    metadata = licensing.get("source_metadata")
    if not isinstance(metadata, dict) or not metadata:
        raise SystemExit(
            "activity retrieval runtime index must record per-source "
            "release/license/source-manifest metadata"
        )
    missing = sorted(set(counts) - set(metadata))
    if missing:
        raise SystemExit(
            "activity retrieval runtime index is missing per-source "
            "release/license/source-manifest metadata: " + ",".join(missing)
        )
    extra = sorted(set(metadata) - set(counts))
    if extra:
        raise SystemExit(
            "activity retrieval runtime source metadata does not match "
            "source_counts: " + ",".join(extra)
        )
    for source in sorted(counts):
        record = metadata[source]
        if not isinstance(record, dict):
            raise SystemExit(
                f"activity retrieval runtime source metadata is invalid: {source}"
            )
        rows = record.get("rows")
        if not isinstance(rows, int) or isinstance(rows, bool) or rows != counts[source]:
            raise SystemExit(
                "activity retrieval runtime source metadata does not match "
                f"source_counts rows: {source}"
            )
        _nonempty_string_or_list(
            record.get("releases"), f"runtime source {source} release metadata"
        )
        _nonempty_string_or_list(
            record.get("licences"), f"runtime source {source} license metadata"
        )
        _validate_file_record(
            record.get("source_manifest"),
            runtime_index_manifest,
            f"runtime source {source} manifest record",
        )
...
def _validate_runtime_operational_inputs(
    *,
    ...
    _validate_runtime_source_licensing(runtime_index, runtime_index_manifest)
