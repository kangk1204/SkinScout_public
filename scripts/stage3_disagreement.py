...
DISAGREEMENT_SCHEMA_VERSION = "skinscout.dti-disagreement.v1"
...
def source_status_path(psichic_path: Path) -> Path:
    return psichic_path.with_suffix(psichic_path.suffix + ".status.json")


def read_source_status(path: Path, label: str) -> dict[str, object] | None:
    if not path.exists():
        return None
    if path.stat().st_size == 0:
        raise SystemExit(f"{label} is present but empty: {path}")
    try:
...
def rank_targets(df: pd.DataFrame, score_col: str, label: str) -> list[str]:
    ...
    df = df.sort_values(
        [score_col, "target_id"], ascending=[False, True], kind="mergesort"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "PSICHIC source-status manifest; defaults to "
            "<psichic>.status.json when present."
        ),
    )
    ...
    status_path = args.psichic_status or source_status_path(args.psichic)
    psichic_status = read_source_status(status_path, "PSICHIC source status")
    if psichic_status is not None and psichic_status.get("status") == "unavailable":
        # The DTI side was never scored, so no disagreement number can be
        # claimed. Emit an explicit structured state instead of treating the
        # header-only table as an empty but valid ranking, and instead of
        # aborting the run whose other artifacts are still diagnosable.
        payload = {
            "schema_version": DISAGREEMENT_SCHEMA_VERSION,
            "status": "unavailable",
            "claimable": False,
            "reason": str(psichic_status.get("reason") or "psichic_unavailable"),
            "detail": psichic_status.get("detail"),
            "psichic_source_status": psichic_status,
        }
        _write_json_atomic(payload, args.out_json)
        LOG.warning(
            "PSICHIC unavailable (%s); wrote non-claimable disagreement stub",
            payload["reason"],
        )
        return
