...
def ensure_autogrid(micromamba: Path, *, dry_run: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="skinscout-autogrid-") as temporary:
        ...
        # upstream meson.build는 `../ad4_shared/...`를 **빌드 디렉터리** 기준으로
        # 참조한다. 그래서 빌드 디렉터리가 소스 루트의 자식이어야 한다 - 소스 밖
        # 형제 디렉터리에서 빌드하면 `paramdat2h.csh: No such file or directory`로
        # 죽는다(실측). meson은 소스 안 빌드 디렉터리를 허용한다.
        build = source / "build"
        run(
            [
                str(micromamba),
...
QUALIFICATION_REQUIRED_OUTPUTS: dict[str, frozenset[str]] = {
    "skinscout.structural-qualification.v1": frozenset({
        "daina_scores",
        "daina_metadata",
        "daina_selected",
        "daina_compatibility",
    ...
    "skinscout.report-qualification.v1": frozenset({
        "report_html",
        "fast_manifest",
    }),
}
...
def _qualification_payload_valid(path: Path, schema: str) -> bool:
    required_outputs = QUALIFICATION_REQUIRED_OUTPUTS.get(schema)
...
def run_phase(
    ...
    try:
        if phase == "host_preflight":
            ...
            if args.analog_bundle_manifest_sha256:
                command += ["--manifest-sha256", args.analog_bundle_manifest_sha256]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--analog-bundle-manifest-sha256", default=None,
