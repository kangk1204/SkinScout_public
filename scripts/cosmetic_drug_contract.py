...
def expected_decision_from_drug_warnings(
    ...
    tiers: set[str] = set()
    known_tiers = {"STRICT_WARNING", "SCAFFOLD_MATCH", "SOFT_WARNING"}
    for idx, warning in enumerate(warnings):
        ...
        value = tier.strip() if isinstance(tier, str) else ""
        if value not in known_tiers:
            # 알 수 없는 tier를 조용히 버리면 corrupt 목록이 PROCEED로 재계산된다.
            raise CosmeticDrugContractError(
                f"drug-avoidance warnings entry {idx} has unknown tier {tier!r}"
            )
        tiers.add(value)
