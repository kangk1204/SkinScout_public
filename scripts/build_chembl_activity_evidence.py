...
def _coerce_chunk(chunk: pd.DataFrame) -> pd.DataFrame:
    ...
    # D06: pandas float 경유로 '15149688.0'이 되는 식별자를 정수 문자열로 되돌린다.
    # Int64 등 수치 열에는 문자열 accessor를 적용하지 않는다(F05 회귀 방지).
    for column in ("pubmed_id", "document_id", "assay_id", "target_id", "molecule_molregno"):
        if column not in chunk.columns:
            continue
        if pd.api.types.is_numeric_dtype(chunk[column]):
            chunk[column] = chunk[column].astype("Int64").astype("string")
        else:
            chunk[column] = chunk[column].astype("string").str.replace(
