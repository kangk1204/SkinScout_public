...
def test_the_readme_keeps_operator_material_out_of_the_researcher_path() -> None:
    ...
    # 공개 스냅샷에는 문서 맨 위에 "이 저장소는 공개본입니다" 안내 블록이 붙는다.
    # 이 상한이 보는 것은 연구자 경로의 길이이므로 그 블록은 빼고 센다 - 빼지 않으면
    # 같은 README이 로컬과 공개본에서 다르게 판정된다.
    lines = readme.splitlines()
    note_start = next(
        (index for index, line in enumerate(lines) if "이 저장소는 공개본입니다" in line),
        None,
    )
    if note_start is not None:
        note_end = note_start
        while note_end < len(lines) and (
            lines[note_end].startswith(">") or not lines[note_end].strip()
        ):
            note_end += 1
        lines = lines[:note_start] + lines[note_end:]
    assert len(lines) < 930, "README is drifting back toward a manual"
