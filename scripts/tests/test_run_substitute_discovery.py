...
def test_relative_sdf_is_normalized_once_for_parent_anchor_and_provenance(
    ...
    # 적용 범위 판정이 SDF를 실제로 해석하므로, 경로 정규화를 검증하는 이 테스트도
    # 텍스트 자리표시자가 아니라 파싱 가능한 분자를 쓴다(에탄올 vs 메탄올).
    caller_molblock = Chem.MolToMolBlock(Chem.MolFromSmiles("CCO"))
    sdf.write_text(caller_molblock)
    execution_dir = tmp_path / "execution"
    execution_dir.mkdir()
    (execution_dir / "compound.sdf").write_text(
        Chem.MolToMolBlock(Chem.MolFromSmiles("CO"))
    )
    ...
    def check_provenance(run_dir: Path, requested: object) -> None:
        ...
        assert args.sdf.read_bytes() == caller_molblock.encode()
