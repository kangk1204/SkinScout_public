...
def test_generic_description_tokens_are_ambiguous_with_candidates() -> None:
    """`collagen`/`protein`/`factor` describe many entries. Picking whichever
    record was read first would invent a mapping; report the candidates."""
    for query in ("Collagen", "Protein", "Factor"):
        entry, candidates = et.resolve_target(query)
        assert entry is None, query
        assert len(candidates) > 1, query
        assert len(candidates) == len(set(candidates)), query

    entry, candidates = et.resolve_target("Collagen")
    assert {"P02461", "P08123", "Q9UMD9"} <= set(candidates)


def test_unique_description_token_resolves_and_ambiguous_token_does_not() -> None:
    entries = [
        et.LexiconEntry("P1", "GENE1", "Alpha collagenase"),
        et.LexiconEntry("P2", "GENE2", "Beta protease"),
        et.LexiconEntry("P3", "GENE3", "Gamma collagenase"),
    ]
    # 설명문 토큰은 exact dictionary에 들어가지 않는다.
    assert "collagenase" not in et._build_lexicon(entries)
    tokens = et._description_tokens(entries)
    assert tokens["collagenase"] == ["P1", "P3"]
    assert tokens["protease"] == ["P2"]
