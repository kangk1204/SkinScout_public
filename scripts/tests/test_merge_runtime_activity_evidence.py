...
def test_the_tier_that_ships_carries_no_share_alike_obligation() -> None:
    ...
    this tier, so a share-alike licence appearing here would put an obligation
    into every run without anyone choosing it."""
    manifest = json.loads((TIERS[0] / "manifest.json").read_text(encoding="utf-8"))

    assert manifest["sources_tier"] == "permissive"
    assert manifest["sources"] == ["bindingdb_own"]
    for licence in manifest["licences"]:
