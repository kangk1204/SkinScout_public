...
def test_automatic_literature_efficacy_is_flagged_for_review_on_screen() -> None:
    """The UI must read the same review flag the summary and report write."""
    javascript = _read("app.js")

    assert "binding.skin_efficacy_requires_review === true" in javascript
    assert (
        '"skin efficacy evidence is automatic literature co-occurrence '
        'and requires review"' in javascript
    )
    assert "피부 효능 근거가 자동 문헌 공출현이라 사람이 검토해야 합니다." in javascript
