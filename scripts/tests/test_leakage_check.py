...
@pytest.fixture
def sequence_stub(monkeypatch: pytest.MonkeyPatch):
    """Deterministic alignment score for tests that do not exercise MMseqs."""

    def _stub(_uid: str, _db: Path, query_sequence: str | None = None):
        if query_sequence is None or not str(query_sequence).strip():
            return None
        return 0.9

    monkeypatch.setattr(leakage_check, "mmseqs_max_seq_id", _stub)
    return _stub
...
def test_sequence_axis_reports_incomplete_without_mmseqs(
    tmp_path: Path,
    ...
    assert mmseqs_max_seq_id("P12345", fasta, query_sequence="MABCXE" * 4) is None
...
def test_sequence_identity_flags_domain_fragment_containment_without_mmseqs(
    ...
    score = mmseqs_max_seq_id("P12345", fasta, query_sequence="MABCDE" * 4)

    assert score is None, (
        "containment must not be substituted for an alignment identity"
    )
...
def test_sequence_axis_flags_short_query_contained_in_long_reference_without_mmseqs(
    ...
    assert pd.isna(out.loc[0, "seq_id"])
    assert out.loc[0, "audit_status"] == "incomplete"
    assert "sequence" in out.loc[0, "missing_axes"]
    assert not out.loc[0, "leak_flag"]
...
def test_sequence_axis_uses_mmseqs_when_available(
    ...
    fake_mmseqs.write_text(
        ...
        "printf 'P12345\\tref1\\t95.0\\t100\\t1.0\\t1.0\\n' > \"$4\"\n"
    )
    ...
    score = mmseqs_max_seq_id("P12345", fasta, query_sequence="MABCXE" * 10)
...
def test_sequence_axis_fails_closed_when_available_mmseqs_fails(
    ...
    with pytest.raises(RuntimeError, match="MMseqs sequence leakage search failed"):
        mmseqs_max_seq_id("P12345", fasta, query_sequence="MABCXE" * 10)
...
def test_leakage_audit_ok_when_all_axes_have_references(
    tmp_path: Path, sequence_stub
) -> None:
    seq = tmp_path / "training_cutoff_seqs.fasta"
    seq.write_text(">ref\nMABCDE\n")
    lig = tmp_path / "ligands.smi"
...
def test_sealed_discovery_audit_rejects_changed_active_source(
    tmp_path: Path, sequence_stub
) -> None:
    cases = tmp_path / "cases.csv"
    seq = tmp_path / "training_cutoff_seqs.fasta"
...
def test_sealed_discovery_audit_rejects_binding_tamper(
    tmp_path: Path, sequence_stub
) -> None:
    cases = tmp_path / "cases.csv"
    seq = tmp_path / "training_cutoff_seqs.fasta"
