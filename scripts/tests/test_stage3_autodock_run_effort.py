...
def test_docking_seed_is_deterministic_per_ligand_and_target(tmp_path: Path) -> None:
    """AutoDock-GPU 기본 시드는 time+pid라, 지정하지 않으면 재실행마다 달라진다."""
    from stage3_autodock_run import docking_seed

    ligand = tmp_path / "ligand.pdbqt"
    ligand.write_text("ATOM      1  C   LIG A   1       0.000   0.000   0.000\n", encoding="utf-8")

    same = docking_seed(ligand, "P14679")
    assert same == docking_seed(ligand, "P14679")
    assert same != docking_seed(ligand, "P08253")

    ligand.write_text("ATOM      1  O   LIG A   1       0.000   0.000   0.000\n", encoding="utf-8")
    assert docking_seed(ligand, "P14679") != same


def test_autodock_invocation_passes_the_fixed_seed(tmp_path: Path, monkeypatch) -> None:
    import stage3_autodock_run as mod

    ligand = tmp_path / "ligand.pdbqt"
    ligand.write_text("ATOM\n", encoding="utf-8")
    map_file = tmp_path / "T.maps.fld"
    map_file.write_text("fld\n", encoding="utf-8")
    ...
    mod.run_autodock_gpu_one(
        ligand, map_file, 4, "ad", tmp_path / "work", "autodock_gpu_128wi", target_id="T"
    )

    cmd = captured["cmd"]
    assert "--seed" in cmd
    seed = cmd[cmd.index("--seed") + 1]
    assert seed == str(mod.docking_seed(ligand, "T"))
    assert seed.isdigit()
