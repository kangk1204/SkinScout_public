...
DEV_SPECS_EXCLUDED = (
    ...
    "docs/SKINSCOUT_READONLY_PRECISION_AUDIT_20260918.md",
    "docs/SKINSCOUT_POSTFIX_DEEP_REVIEW_20260918.md",
    "docs/SKINSCOUT_CURRENT_FULL_AUDIT_20260918.md",
)
...
def test_no_campaign_analysis_artifact_reaches_it(snapshot: Path) -> None:
    """Discovery campaign outputs (run ids, candidate ids, seed structures) stay private.

    The public tree must carry the tool, not the analysis: candidate molecules,
    docking scores and campaign identifiers belong to the user's private
    workspace. Patterns are assembled from fragments so this test file itself
    cannot trip the scan.
    """
    run_id = re.compile(r"\bsmiles_[0-9a-f]{12}\b")
    candidate_id = re.compile(r"\b(?:BRICS_[0-9a-f]{6}|LINK_[0-9]{2})\b")
    workspace_labels = (
        "collaborator_" + "package",
        "100_" + "homework",
        "SkinScout_" + "biomarker",
    )
    offenders: list[str] = []
    for path in snapshot.rglob("*"):
        if not path.is_file():
        ...
        hit = (
            run_id.search(text)
            or candidate_id.search(text)
            or any(label in text for label in workspace_labels)
        )
        if hit:
            offenders.append(str(path.relative_to(snapshot)))
    assert offenders == [], f"campaign artifacts in public snapshot: {offenders}"
