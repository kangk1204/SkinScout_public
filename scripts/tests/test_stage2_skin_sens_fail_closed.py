...
def write_consensus_inputs(tmp_path: Path) -> None:
    ...
    write_json(
        ...
        {"smiles": "CCO", "status": "ok", "skin_sens_call": "negative",
         "applicability_domain": {"status": "inside"}},
    )
    write_json(
    ...
    write_json(
        ...
        {"smiles": "CCO", "status": "ok", "consensus_call": "negative"},
    )
...
def test_consensus_rejects_missing_skin_sens_evidence_by_default(tmp_path: Path) -> None:
    ...
    write_json(admet, {"smiles": "CCO", "status": "ok",
                       "predictions": {"Skin_Reaction": 0.1}})
    write_json(husspred, {"smiles": "CCO", "status": "unavailable"})
    write_json(stoptox, {"smiles": "CCO", "status": "ok",
                         "skin_sens_call": "negative"})
    write_json(pred_skin, {"smiles": "CCO", "status": "ok",
...
def test_consensus_rejects_invalid_skin_sens_call_without_stale_outputs(
    ...
    write_json(
        tmp_path / "pred_skin.json",
        {"smiles": "CCO", "status": "ok", "consensus_call": "maybe"},
    )
    (tmp_path / "report.json").write_text("stale\n")
    (tmp_path / "decision.txt").write_text("stale\n")
...
def test_consensus_rejects_out_of_range_skin_sens_probability_without_stale_outputs(
    ...
    write_json(
        tmp_path / "husspred.json",
        {"smiles": "CCO", "status": "ok", "skin_sens_probability": 1.2,
         "applicability_domain": {"status": "inside"}},
    )
    (tmp_path / "report.json").write_text("stale\n")
    (tmp_path / "decision.txt").write_text("stale\n")
...
def test_web_model_smiles_drops_explicit_hydrogens(tmp_path: Path) -> None:
    """Stage 1 SDF의 명시적 수소를 그대로 보내면 문자열이 몇 배로 길어진다.

    Pred-Skin API는 500자 제한이라, 수소가 붙은 채로 보내면 422로 거부된다
    (실측: 펩타이드성 화합물 786자). 세 웹 모델 모두 정규 SMILES를 보낸다.
    """
    from stage2_husspred import smiles_from_sdf as husspred_smiles
    from stage2_pred_skin import smiles_from_sdf as pred_skin_smiles
    from stage2_stoptox import smiles_from_sdf as stoptox_smiles

    sdf = tmp_path / "explicit_h.sdf"
    writer = Chem.SDWriter(str(sdf))
    writer.write(Chem.AddHs(Chem.MolFromSmiles("CCO")))
    writer.close()

    for function in (pred_skin_smiles, stoptox_smiles, husspred_smiles):
        smiles = function(sdf)
