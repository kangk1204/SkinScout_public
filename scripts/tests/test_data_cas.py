...
@pytest.mark.parametrize(
    "name",
    ["../evil", "a/b", "a\\b", "/absolute", "..", ".", "", " lead", "trail "],
)
def test_manifest_rejects_unsafe_pack_names(name: str) -> None:
    """Pack name becomes one path component under store/staging; a signed but
    unsafe name must fail at schema validation, before staging paths exist."""
    manifest = _manifest("models/a.txt", b"abc")
    manifest["packs"][0]["name"] = name

    with pytest.raises(data_cas.CasError, match="pack name"):
        data_cas.validate_manifest(manifest, verifier=_verifier())


@pytest.mark.parametrize(
    "version",
    ["../evil", "1.0/2", "a\\b", "/absolute", "..", "", "1.0 ", " 1.0"],
)
def test_manifest_rejects_unsafe_pack_versions(version: str) -> None:
    manifest = _manifest("models/a.txt", b"abc")
    manifest["packs"][0]["version"] = version

    with pytest.raises(data_cas.CasError, match="pack version"):
        data_cas.validate_manifest(manifest, verifier=_verifier())


def test_normal_pack_name_and_version_pass_validation() -> None:
    manifest = _manifest("models/a.txt", b"abc")
