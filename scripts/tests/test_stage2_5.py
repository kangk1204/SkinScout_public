...
def test_cosing_query_is_neutralized_before_matching(tmp_path) -> None:
    """Stage 1의 pH 7.4 이온 형태 SDF도 중성 InChIKey로 질의해야 한다.

    이온화된 채로 계산하면 같은 분자가 exact가 아니라 ANALOG/NEW로 떨어진다.
    """
    from rdkit import Chem

    from stage2_5_cosing_match import _load_query

    path = tmp_path / "phenolate.sdf"
    writer = Chem.SDWriter(str(path))
    writer.write(Chem.MolFromSmiles("[O-]c1ccccc1"))
    writer.close()

    key, _ = _load_query(path)
    assert key == Chem.MolToInchiKey(Chem.MolFromSmiles("Oc1ccccc1"))


def test_cosing_reference_standard_key_rescues_a_stale_stored_key() -> None:
    """참조의 저장 inchikey가 낡아도 표준화 키로 exact를 찾아야 한다.

    CosIng parquet의 inchikey는 stage0가 표준화 전에 만든 값이라, 실행 경로가
    중성 표준화 키로 질의하면 같은 분자를 놓친다(하이드록시레스베라트롤 사례).
    """
    import pandas as pd
    from rdkit import Chem
    from rdkit.Chem import rdFingerprintGenerator

    from stage2_5_cosing_match import _attach_standard_keys, _best_match

    neutral = Chem.MolFromSmiles("Oc1ccc(/C=C/c2ccc(O)cc2)cc1")
    key = Chem.MolToInchiKey(neutral)
    frame = _attach_standard_keys(pd.DataFrame([{
        "inchikey": "STALE-0000-0000",
        "ecfp4": ["0"] * 32,
        "inci_name": "TEST",
        "functions": "BLEACHING",
        "smiles": Chem.MolToSmiles(neutral),
    }]))
    assert frame.iloc[0]["standard_key"] == key

    bv = rdFingerprintGenerator.GetMorganGenerator(radius=2, fpSize=2048).GetFingerprint(neutral)
    match = _best_match(key, bv, frame, 0.85, 0.65)
    assert match.level == "EXACT"
    assert match.inci == "TEST"
