...
def test_the_index_a_run_actually_reads_carries_no_share_alike_licence() -> None:
    ...
    # ChEMBL is the base mirror and always carries CC BY-SA 3.0; the permissive
    # tier choice is about not adding GtoPdb's ODbL / CC BY-SA 4.0 on top.
    # F24 made the ledger per-source, so name each forbidden obligation.
    for licence in licences:
        assert "ODbL" not in licence and "CC BY-SA 4.0" not in licence, (
            f"the default run path would inherit an unrequested share-alike obligation: {licence}"
        )
    ...
    for source, record in (licensing.get("source_metadata") or {}).items():
        assert "GtoPdb" not in source, manifest_path
        for licence in record.get("licences") or []:
            assert "ODbL" not in licence and "CC BY-SA 4.0" not in licence, (
                f"{source} would inherit an unrequested share-alike obligation: {licence}"
            )
