...
For each candidate receptor:
  • Crop residues within `--crop-radius` Å of the P2Rank pocket centroid when
    sequence length > `--max-residues` (12 GB GPU host constraint).
...
def call_boltz(receptor_pdb: Path, ligand_sdf: Path, workdir: Path,
               max_residues: int, crop_radius: float) -> float | None:
