...
from residue_mapping import (  # noqa: E402
    load_mapping_record,
    mapping_path_for_pdb,
...
FRESHNESS_SCHEMA = "skinscout.bioemu_ensemble_freshness.v1"
FRESHNESS_FILENAME = "ensemble_freshness.json"
...
def _count_trajectory_frames(trajectories: list[Path], topology: Path) -> int | None:
    """Total frames across XTC trajectories, or None if it cannot be verified."""
    try:
        import mdtraj as md
    except ImportError:
        return None
    ...
    for xtc in trajectories:
        try:
            total += int(md.load(str(xtc), top=str(topology)).n_frames)
        except Exception as exc:  # noqa: BLE001 - mdtraj exceptions are broad
            LOG.warning("Could not read fresh trajectory %s: %s", xtc, exc)
            return None
    return total


def validate_fresh_ensemble(out_dir: Path) -> tuple[dict[str, Any] | None, str]:
    """Validate topology, trajectory/conformer files, frame counts, and hashes.

    Only an ensemble that this invocation actually produced is allowed to be
    published. A zero-output returncode-0 run leaves no topology and is
    rejected here instead of falling back to whatever a previous run left.
    """
    topology = out_dir / "topology.pdb"
    if not nonempty(topology):
        return None, f"missing or empty topology.pdb in {out_dir}"
    trajectories = [path for path in sorted(out_dir.glob("*.xtc")) if nonempty(path)]
    conformers = [
        path
        for path in sorted(out_dir.glob("*.pdb"))
        if path.name != "topology.pdb" and nonempty(path)
    ]
    if not trajectories and not conformers:
        return None, "BioEmu produced no trajectory or conformer files"
    if trajectories:
        frames = _count_trajectory_frames(trajectories, topology)
        if frames is None:
            return None, "trajectory frame count could not be verified"
    else:
        frames = sum(_count_pdb_models(path) for path in conformers)
    if frames < 1:
        return None, "BioEmu ensemble contains zero frames"
    return {
        "total_frames": frames,
        "topology": _file_fingerprint(topology),
        "trajectories": [_file_fingerprint(path) for path in trajectories],
        "conformers": [_file_fingerprint(path) for path in conformers],
    }, ""
...
def write_freshness_manifest(
    ...
    *,
    sequence_sha256: str,
    n_conf_requested: int,
) -> Path:
    record = {
        "schema_version": FRESHNESS_SCHEMA,
        "generated_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "sequence_sha256": sequence_sha256,
        "n_conf_requested": n_conf_requested,
        "total_frames": report["total_frames"],
        "topology": report["topology"],
        "trajectories": report["trajectories"],
        "conformers": report["conformers"],
    }
    path = out_dir / FRESHNESS_FILENAME
...
def verify_freshness_manifest(conformer_dir: Path) -> tuple[bool, str]:
    """Re-hash the published ensemble against its freshness manifest."""
    path = conformer_dir / FRESHNESS_FILENAME
    if not nonempty(path):
        return False, f"missing ensemble freshness manifest: {path}"
    try:
        record = json.loads(path.read_text())
    except (OSError, json.JSONDecodeError) as exc:
        return False, f"invalid ensemble freshness manifest: {exc}"
    if not isinstance(record, dict) or record.get("schema_version") != FRESHNESS_SCHEMA:
        return False, "unsupported ensemble freshness manifest schema"
    total_frames = record.get("total_frames")
    if not isinstance(total_frames, int) or isinstance(total_frames, bool) or total_frames < 1:
        return False, "ensemble freshness manifest reports zero frames"
    entries = [record.get("topology")]
    entries.extend(record.get("trajectories") or [])
    entries.extend(record.get("conformers") or [])
    if not any(isinstance(entry, dict) for entry in entries):
        return False, "ensemble freshness manifest lists no files"
    for entry in entries:
        if not isinstance(entry, dict):
            return False, "ensemble freshness manifest contains a malformed entry"
        name = entry.get("name")
        if not isinstance(name, str) or not name or Path(name).name != name:
            return False, f"invalid ensemble file name: {name!r}"
        candidate = conformer_dir / name
        if not nonempty(candidate):
            return False, f"ensemble file is missing or empty: {name}"
        if candidate.stat().st_size != entry.get("bytes") or _sha256_file(candidate) != entry.get("sha256"):
            return False, f"ensemble file hash changed since validation: {name}"
    return True, ""


def _publish_directory(staged: Path, target: Path) -> None:
    """Replace the published ensemble with the validated staging directory."""
...
def sampling_sequence(
    ...
    """Return the mapped observed sequence, or a rejection reason.

    The residue mapping record written by Stage 4 binds this PDB's hash, the
    observed spans, and the canonical sequence hash. A discontinuous observed
    chain is refused: BioEmu must not sample a sequence in which residue 10 is
    directly followed by residue 20.
    """
    path = mapping_path or mapping_path_for_pdb(receptor)
    record = load_mapping_record(path)
    if record is None:
        return None, f"missing residue mapping record: {path}"
    errors = validate_mapping_record(receptor, record)
    if errors:
        return None, "; ".join(errors)
    sequence = record.get("observed_sequence")
    if not isinstance(sequence, str) or not sequence:
        return None, "residue mapping record has no observed sequence"
...
def run_bioemu(
    ...
    *,
    ...
    """Sample BioEmu in a fresh staging directory and publish only if valid.

    Running in the live target directory made a returncode-0 invocation with no
    output read the previous run's ensemble as if it were current. Staging
    means the old files are never visible to validation, and a same-sequence
    result from a different seed or package version cannot be accepted as this
    run's output.
    """
    ...
    if not bioemu_available():
        return False
    sequence, rejection = sampling_sequence(receptor, mapping_path)
    if sequence is None:
        LOG.warning(
            "BioEmu input refused for %s: %s", receptor.name, rejection
        )
        return False
    out_dir.parent.mkdir(parents=True, exist_ok=True)
    staged = Path(
        tempfile.mkdtemp(prefix=f".{out_dir.name}.bioemu_", dir=out_dir.parent)
    )
    try:
        res = subprocess.run(
            bioemu_command(sequence, staged, n_conf),
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
        ...
        if res.returncode != 0:
            ...
            return False
        report, reason = validate_fresh_ensemble(staged)
        if report is None:
            LOG.warning(
                "BioEmu produced no fresh validated ensemble for %s: %s",
                receptor.name,
                reason,
            )
            return False
        write_freshness_manifest(
            ...
            n_conf_requested=n_conf,
        )
        _publish_directory(staged, out_dir)
        LOG.info(
            "Published fresh BioEmu ensemble for %s: %d frame(s)",
            receptor.name,
            report["total_frames"],
        )
        return True
...
def cluster_conformers(conformer_dir: Path, k: int) -> list[Path]:
    ...
    fresh, freshness_reason = verify_freshness_manifest(conformer_dir)
    if not fresh:
        LOG.warning(
            "Refusing to cluster %s: %s", conformer_dir, freshness_reason
        )
        return []
...
def main() -> None:
    ...
    for uid in boltz["target_id"].astype(str):
        ...
        mapping_path = args.struct_dir / uid / f"{uid}_input_residue_mapping.json"
        if not nonempty(mapping_path):
            raise SystemExit(
                f"BioEmu receptor residue mapping is missing or empty for {uid}: "
                f"{mapping_path}; Stage 4 must prepare it before sampling"
            )
