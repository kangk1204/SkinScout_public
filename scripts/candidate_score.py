...
def score_candidates(frame: pd.DataFrame,
    ...
    if weights:
        unknown = sorted(set(weights) - set(w))
        if unknown:
            raise ValueError(f"알 수 없는 가중치 키입니다: {', '.join(unknown)}")
        for key, value in weights.items():
            if isinstance(value, bool):
                raise ValueError(f"가중치 {key}는 bool일 수 없습니다: {value!r}")
            number = float(value)
            if not math.isfinite(number) or number < 0:
                raise ValueError(f"가중치 {key}는 유한한 비음수여야 합니다: {value!r}")
            w[key] = number
    total_weight = sum(w.values())
    if not math.isfinite(total_weight) or total_weight <= 0:
        raise ValueError(f"가중치 합은 유한한 양수여야 합니다: {w}")
