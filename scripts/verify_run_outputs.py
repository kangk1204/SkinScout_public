...
FAST_TARGET_ARTIFACTS = {
    ...
    "target_fast_band_reranked_full": "03_targets/mode_fast/daina_band_reranked_targets.csv",
}
...
def _verify_admet(
    ...
    *,
    ...
    # F30: applicability-limited models are available, not missing, but their
    # vote is withheld from the PASS basis; degraded runs must accept the gap.
    limited_models = skin_sens.get("applicability_limited_models") or []
    if not isinstance(limited_models, list):
        checks.append(_invalid("ADMET and skin-sens report", path, "applicability_limited_models must be a list"))
        limited_models = []
    for model in SKIN_SENS_MODELS:
        call = skin_sens.get(model)
        if not isinstance(call, str) or call.strip() not in SKIN_SENS_CALLS:
            allowed_absent = allow_degraded and (
                (isinstance(missing_models, list) and model in missing_models)
                or model in limited_models
            )
...
def _verify_daina_fast_target_intermediates(
    ...
    _verify_band_reranked_targets(base, canonical, checks)


def _verify_band_reranked_targets(
    base: Path, canonical: pd.DataFrame, checks: list[OutputCheck]
) -> None:
    """Recompute the full policy, then verify its reader-facing projection."""
    from stage3_band_rerank import rerank_policy_error

    full_path = base / "daina_band_reranked_targets.csv"
    full, full_check = _read_csv(full_path, "complete Daina policy targets")
    full_ok = _append_if_bad(checks, full_check)
    if full_ok:
        detail = rerank_policy_error(full, canonical)
        if detail:
            checks.append(_invalid("complete Daina policy targets", full_path, detail))
            full_ok = False

    delivered_path = base / "top50_band_reranked.csv"
    delivered, delivered_check = _read_csv(delivered_path, "reader-facing Daina band-reranked targets")
    if _append_if_bad(checks, delivered_check) and full_ok:
        expected = full.head(50).reset_index(drop=True)
        if len(expected) != 50:
            checks.append(_invalid("reader-facing Daina band-reranked targets", delivered_path,
                                   "complete policy must supply 50 delivered targets"))
            return
        try:
            pd.testing.assert_frame_equal(delivered, expected, check_dtype=False, check_exact=True)
        except AssertionError:
            checks.append(_invalid("reader-facing Daina band-reranked targets", delivered_path,
                                   "top50 must be the exact first 50 rows of the verified complete policy"))
