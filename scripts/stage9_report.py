...
from mmgbsa_policy import annotate_selection
...
def _validate_skin_sens_report_calls(
    ...
    *,
    ...
    # F30: applicability-limited models are available evidence whose vote is
    # blocked from the PASS basis; their absent call is not a missing model.
    raw_limited = skin_sens.get("applicability_limited_models") or []
    if not isinstance(raw_limited, list):
        raise SystemExit(
            "ADMET report skin_sens.applicability_limited_models must be a list"
        )
    limited = {str(model).strip() for model in raw_limited if str(model).strip()}
    for model in SKIN_SENS_MODELS:
        call = skin_sens.get(model)
        if isinstance(call, str) and call.strip() in SKIN_SENS_CALLS:
            continue
        if allow_degraded and (model in missing or model in limited):
            continue
...
def _skin_sens_evidence(args: argparse.Namespace) -> list[dict[str, str]]:
    ...
    for model in SKIN_SENS_MODELS:
        ...
        applicability_text = "n/a"
        if model == "husspred":
            applicability = source.get("applicability_domain")
            if isinstance(applicability, dict):
                applicability_text = (
                    _text_or_none(applicability.get("status")) or "unknown"
                )
        rows.append({
            "model": model,
            "status": status,
            ...
            "applicability_domain": applicability_text,
        })
    return rows
...
def _rerank_policy_metadata(frame: pd.DataFrame) -> dict[str, Any] | None:
    present = RERANK_POLICY_COLUMNS & set(frame.columns)
    if not present:
        return None
    missing = sorted(RERANK_POLICY_COLUMNS - set(frame.columns))
    if missing:
        raise SystemExit(f"Stage 3 ranked targets have incomplete rerank policy metadata: {missing}")
    ...
    for index, row in frame.iterrows():
        ...
        if isinstance(raw_enabled, str) and raw_enabled.strip().lower() in {"true", "false"}:
            ...
            raise SystemExit(
                "Stage 3 rerank_policy_enabled must be boolean at row "
                f"index {index}"
            )
        status = _text_or_none(row["rerank_policy_status"])
        if status not in {"experimental", "disabled"}:
            raise SystemExit(
                "Stage 3 rerank_policy_status must be experimental or disabled"
            )
        keep = _row_int(row, "rerank_keep")
        band_end = _row_int(row, "rerank_band_end")
        if keep is None or keep < 0 or band_end is None or band_end < 0:
            raise SystemExit("Stage 3 rerank policy bounds must be non-negative integers")
        if enabled != (status == "experimental"):
            raise SystemExit("Stage 3 rerank policy enabled/status values are inconsistent")
        if enabled != (keep < band_end):
            raise SystemExit("Stage 3 rerank policy bounds do not match enabled flag")
        rows.append((enabled, status, keep, band_end))
    unique = set(rows)
    if len(unique) != 1:
        raise SystemExit("Stage 3 rerank policy metadata must be identical on every row")
...
def _skin_context_summary(stage3_top: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "skin_efficacy_requires_review": efficacy_supported,
...
def _overall_decision_summary(
    *,
    ...
    # Automatic gene-keyword co-occurrence is a literature-search signal, never
    # an approved efficacy claim. Without this the report could say PASS and
    # claimable while the same panel said the evidence required review.
    efficacy_claim_allowed = decision == "PASS" and not (
        skin_context.get("skin_efficacy_requires_review") is True
    )
    if not efficacy_claim_allowed:
        reasons.append(
...
def _report_decision_html(
    *,
    ...
    if isinstance(rerank_policy, dict):
        if rerank_policy.get("enabled") is True:
            rerank_policy_html = (
                "<p><strong>Experimental target reranking is enabled.</strong> "
                "This ranking is diagnostic and nonclaimable until current-policy "
                "docking validation passes.</p>"
            )
        else:
            rerank_policy_html = (
                "<p>Target reranking policy: <code>disabled</code>; canonical recipe "
    ...
    return (
        ...
        f" · Skin efficacy supported: <strong>{'yes' if skin_context['skin_efficacy_supported'] else 'no'}</strong>"
        f" · Skin efficacy requires review: <strong>{'yes' if skin_context.get('skin_efficacy_requires_review') else 'no'}</strong>"
        f" · Efficacy claim allowed: <strong>{'yes' if overall.get('efficacy_claim_allowed') else 'no'}</strong></p>"
        "<p>Skin expression basis: <code>relative within-build expression context</code>"
        f" · heuristic context threshold: <code>{SKIN_EXPRESSION_SUPPORTED_THRESHOLD:.2f}</code>"
        " · this threshold is not validated evidence of efficacy.</p>"
        "<p>KG efficacy labels: <code>automatic gene-keyword co-occurrence</code>"
        " · review required before interpreting mechanism, direction, or compound efficacy.</p>"
...
def render(args: argparse.Namespace) -> str:
    ...
    rerank_policy = _rerank_policy_metadata(stage3_top)
    ...
    if _has_physics_args(args):
        ...
        # 0/positive ΔTOTAL values are valid computed results. Display them
        # with an explicit selection decision instead of reporting them as
        # calculation errors; only negative values advance to QM.
        mmgbsa = annotate_selection(mmgbsa)
