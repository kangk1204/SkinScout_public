...
Per receptor:
  ...
  • If the parsed sequence exceeds --max-residues, crop to residues within
    --crop-radius Å of the Stage 4 pocket centre and record the original →
    cropped residue mapping. Without a usable pocket box the target is rejected
    with an explanatory error rather than truncated.
...
Output:
    boltz2_report.tsv columns:
      ...
      affinity_log_uM, pb_valid, kept, n_input_residues, crop_mode, crop_map
"""
...
def run_boltz2(receptor_pdb: Path, ligand_sdf: Path, out_dir: Path,
               n_seeds: int, n_recyc: int,
               *, crop_map: Path | None = None) -> dict | None:
    ...
    try:
        write_affinity_yaml(receptor_pdb, ligand_sdf, input_yaml, crop_map=crop_map)
...
def _n_residues(payload: dict, receptor_pdb: Path, target_id: str) -> int:
    ...
    # Counting observed residues does not require a continuous chain; a mapped
    # pocket crop is deliberately discontinuous and its crop map documents it.
    seq = concatenated_pdb_sequence(receptor_pdb)
    if seq:
        return len(seq)
    raise SystemExit(
...
def _pocket_box_path(row) -> Path | None:
    """Stage 4's pocket box for this target, when the manifest carries one."""
    try:
        value = row.get("pocket_box_json")
    ...
    path = Path(text)
    if not nonempty(path):
        LOG.warning("Pocket box for %s is missing or empty: %s", row.get("target_id"), path)
        return None
    return path
        ...
        for _, row in manifest.iterrows():
            ...
            try:
                model_receptor, crop_meta = prepare_receptor_for_boltz(
                    ...
                    out_dir=target_out,
                    label=uid,
                    pocket_path=_pocket_box_path(row),
                )
            except ReceptorLengthError as exc:
                raise SystemExit(
                    f"Boltz-2 receptor length cap exceeded: {exc}"
                ) from exc
            if crop_meta["crop_mode"] == "pocket_crop":
                LOG.info(
                    "Cropped %s from %d to %d residues around %s; mapping: %s",
                    uid,
                    crop_meta["input_residues"],
                    crop_meta["model_residues"],
                    [round(v, 2) for v in crop_meta["pocket_center"]],
                    crop_meta["crop_map"],
                )
            payload = run_boltz2(model_receptor, args.ligand_sdf, target_out,
                                 args.num_seeds, args.num_recycles,
                                 crop_map=(
                                     Path(crop_meta["crop_map"])
                                     if crop_meta["crop_map"]
                                     else None
                                 ))
            if payload is None:
            ...
            n_residues = _n_residues(payload, model_receptor, uid)
            kept = (
            ...
            rows.append([uid, row["source"], payload["complex_pdb"], n_residues,
                         ...
                         "yes" if kept else "no",
                         str(crop_meta["input_residues"]),
                         str(crop_meta["crop_mode"]),
                         str(crop_meta["crop_map"])])
    ...
    try:
        with tmp_report.open("w", newline="") as fh:
            ...
            w.writerow(["target_id", "source", "complex_pdb", "n_residues", "iptm",
                        ...
                        "n_input_residues", "crop_mode", "crop_map"])
            w.writerows(rows)
