...
def _build_lexicon(entries: list[LexiconEntry]) -> dict[str, str]:
    """유전자·UniProt·명시적 별칭 → UniProt 계정번호. 키는 소문자.

    설명문 토큰은 여기 넣지 않는다. `collagen`, `protein`, `factor` 같은 일반
    토큰은 여러 표적에 걸쳐 있어, 먼저 온 항목으로 setdefault하면 임의 매핑이
    된다. 그런 토큰은 `_description_tokens`에서 모호성 판정에만 쓴다.
    """
...
def _description_tokens(entries: list[LexiconEntry]) -> dict[str, list[str]]:
    """설명문의 단어 → 그 단어를 쓰는 UniProt 목록(정렬·중복 제거)."""
    tokens: dict[str, set[str]] = {}
    for entry in entries:
        for token in re.split(r"\s+", entry.description.lower()):
            if len(token) >= 4 and token.isalpha():
                tokens.setdefault(token, set()).add(entry.uniprot)
    return {token: sorted(values) for token, values in tokens.items()}
...
def resolve_target(query: str) -> tuple[LexiconEntry | None, list[str]]:
    ...
    # 설명문 한 단어가 여러 표적에 걸치면 하나를 골라 답하지 않는다. 단일
    # 표적만 쓰는 토큰은 그대로 해석한다(기존 동작).
    token_hits = _description_tokens(entries).get(key)
    if token_hits:
        if len(token_hits) == 1:
            uniprot = token_hits[0]
            return by_uniprot.get(uniprot) or LexiconEntry(uniprot, "", ""), []
...
def load_index(index_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # 스키마 버전을 확인하지 않으면 다른 세대의 인덱스를 현재 것으로 읽어 열 의미가
    # 어긋난 결과를 낸다(activity_retrieval_scoring.load_reference_index와 동일 계약).
    try:
        ...
        # `from scripts import explore_target`처럼 패키지로 임포트되면 scripts/가
        # sys.path에 없어 절대 임포트가 실패한다(모듈이 다시 activity_recovery_contracts
        # 등을 절대 임포트하므로 개별 모듈 임포트로는 해결되지 않는다).
        import sys
        ...
        try:
            from activity_retrieval_scoring import (  # type: ignore[no-redef]
                INDEX_SCHEMA,
                PRODUCTION_INDEX_SCHEMA,
                _validate_registered_artifact,
            )
        except ImportError as exc:  # pragma: no cover - 배포 사고 방지용 fail-closed
            raise SystemExit(f"인덱스 스키마 상수를 불러올 수 없습니다: {exc}") from exc
    permitted = {INDEX_SCHEMA, PRODUCTION_INDEX_SCHEMA}
    schema = manifest.get("schema_version")
    if schema not in permitted:
        raise SystemExit(
            f"검색 인덱스 스키마가 맞지 않습니다: {schema!r} (허용: {sorted(permitted)})"
        )
    edges_path = index_dir / "edges.parquet"
    ligands_path = index_dir / "ligands.parquet"
    # role/schema/열이 맞아도 다른 실행의 같은 schema 파일일 수 있다. compound-first
    # reference loader와 같은 계약으로 manifest에 기록된 SHA-256과 행 수를 대조한다
    # (activity_retrieval_scoring.load_reference_index 참조). 본문을 읽기 전에
    # 확인해야 바뀐 bytes가 parquet 파싱 오류가 아니라 manifest 불일치로 보고된다.
    import pyarrow.parquet as pq

    try:
        edge_rows = int(pq.ParquetFile(edges_path).metadata.num_rows)
        ligand_rows = int(pq.ParquetFile(ligands_path).metadata.num_rows)
    except Exception as exc:  # noqa: BLE001 - fail closed, 사용자에게 이유를 보여 준다
        raise SystemExit(f"인덱스 parquet을 읽을 수 없습니다: {edges_path}: {exc}") from exc
    _validate_registered_artifact(manifest, "edges", edges_path, edge_rows)
    _validate_registered_artifact(manifest, "ligands", ligands_path, ligand_rows)
