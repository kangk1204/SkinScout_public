...
def build_snakemake_command(
    ...
    *,
    ...
    extra_values = [*args.extra_config, *_sota_config_entries(args), *immutable_config]
    if os.environ.get("SKINSCOUT_RESULTS_ROOT") and "paths.results_root" not in {
        value.split("=", 1)[0].strip() for value in extra_values
    }:
        extra_values.append(f"paths.results_root={DEFAULT_RESULTS_ROOT}")
    config.extend(_snakemake_config_entries(extra_values))
    if args.allow_safety_degraded:
