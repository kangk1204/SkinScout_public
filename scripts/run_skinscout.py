...
def build_snakemake_command(
    ...
    *,
    ...
    # 마지막 단계에서 서로 다른 디렉터리를 보게 된다. 환경변수가 있으면 설정으로
    # 명시 전달해 한쪽으로 맞춘다.
    if os.environ.get("SKINSCOUT_RESULTS_ROOT") and not any(
        entry.startswith("paths.results_root=") for entry in config
    ):
        config.append(f"paths.results_root={DEFAULT_RESULTS_ROOT}")
