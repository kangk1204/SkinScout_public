...
from data_readiness import (  # noqa: E402
    ...
    resolve_stage0_paths as _resolve_stage0_paths,
...
def _run_data_readiness(
    ...
    *,
    ...
    if config is None:
        config = _ACTIVE_STAGE0_CONFIG
    if config is None:
        artifacts = _required_data_artifacts(preset, mode)
        clean_dir = None
    else:
        artifacts = _required_data_artifacts(preset, mode, config=config)
        clean_dir = _resolve_stage0_paths(config)["alphafold_clean"]
    missing = [
        _failed_check_entry(check)
        for label, path in artifacts
        if (
            check := _check_required_artifact(
                label,
                path,
                clean_dir=clean_dir,
            )
    ...
    if allow_stage0_build and _failed_entries_are_stage0_buildable(missing):
        if config is None:
            ...
            source_missing = _stage0_source_failures_for_build(config=config)
        if source_missing:
...
def _run_model_readiness(
    ...
    if not requirements:
        return
    cmd = model_readiness_command(requirements, readiness_command, use_conda=use_conda)
...
def _run_stage0_claim_quality_readiness(
    *,
    config: dict[str, object] | None = None,
) -> None:
    if config is None:
        config = _ACTIVE_STAGE0_CONFIG
    stage0_paths = _resolve_stage0_paths(config) if config is not None else None
    pdbqt_fraction = None
    pdbqt_count = None
    if config is not None:
        stage0_cfg = config.get("stage0", {})
        if isinstance(stage0_cfg, dict):
            pdbqt_fraction = stage0_cfg.get("pdbqt_min_success_fraction")
            pdbqt_count = stage0_cfg.get("pdbqt_min_success_count")
    checks = stage0_verify.collect_checks(
        ROOT,
        claim_quality=True,
        check_stage0_flag=True,
        paths=stage0_paths,
        pdbqt_min_success_fraction=pdbqt_fraction,
        pdbqt_min_success_count=pdbqt_count,
    )
...
def _workflow_config_override_values(args: argparse.Namespace) -> list[str]:
    """The single override list shared by the Snakemake command and preflights."""
    is_discovery = _profile_from_args(args).evidence_mode.value == "discovery"
    values = [*args.extra_config, *_sota_config_entries(args)]
    if is_discovery:
        values.extend(IMMUTABLE_DISCOVERY_CONFIG)
    if os.environ.get("SKINSCOUT_RESULTS_ROOT") and "paths.results_root" not in {
        value.split("=", 1)[0].strip() for value in values
    }:
        values.append(f"paths.results_root={DEFAULT_RESULTS_ROOT}")
    return values


def _effective_workflow_config(args: argparse.Namespace) -> dict[str, object]:
    """Merge the CLI overrides over the static YAML exactly once.

    Readiness, the Stage 0 verifier, and the Snakemake ``--config`` list all
    consume paths resolved from this single object, so an override can no
    longer mean one path to the DAG and another to the gates.
    """
    _validate_user_config(args.extra_config)
    config = _load_workflow_config()
    values = _workflow_config_override_values(args)
    if getattr(args, "allow_safety_degraded", False):
        values = [*values, *SAFETY_DEGRADED_CONFIG]
    try:
        _apply_config_overrides(config, values)
    except ValueError as exc:
        _die(f"--extra-config could not be merged into workflow config: {exc}")
    return config


# Claims made after Snakemake returns (the allow-stage0-build re-check) run
# through the same no-arg helper the tests stub, so main registers the merged
# config once instead of re-reading YAML behind the caller's back.
_ACTIVE_STAGE0_CONFIG: dict[str, object] | None = None


def _register_active_stage0_config(config: dict[str, object]) -> None:
    global _ACTIVE_STAGE0_CONFIG
    _ACTIVE_STAGE0_CONFIG = config
...
def _target_metadata_sha256(path: Path) -> str | None:
    """Content identity of the display-name metadata that reaches the summary.

    The path alone is not an identity: rewriting the same file with different
    gene/protein names changes every displayed label while leaving the path
    unchanged. Missing or empty metadata is recorded as ``None`` so a manifest
    that never proved content cannot later be verified as if it had.
    """
...
def _run_manifest_payload(
    ...
    config_payload = {
        ...
        # The target metadata path sits in snakemake_config, but the displayed
        # names come from its bytes; bind those bytes into the config identity.
        "target_metadata_sha256": _target_metadata_sha256(args.target_metadata),
...
def _run_output_verification(args: argparse.Namespace) -> None:
    ...
    record: dict[str, object] = {
        ...
        # Display names come from these bytes; a path alone would let a rewrite
        # at the same path pass as the same identity.
        "target_metadata": {
            "path": str(args.target_metadata),
            "sha256": _target_metadata_sha256(args.target_metadata),
        },
...
def _run_output_summary(
    ...
    *,
    ...
    if args.preset == "stage0" or args.dry_run or args.skip_output_verification:
        return
    run_dir = _run_dir(args)
    final_json = run_dir / "run_summary.json"
    final_md = run_dir / "run_summary.md"
    staging_dir: Path | None = None
    if atomic_replace:
        run_dir.mkdir(parents=True, exist_ok=True)
        staging_dir = Path(
            tempfile.mkdtemp(prefix=".run_summary_stage_", dir=run_dir)
        )
        out_json = staging_dir / final_json.name
        out_md = staging_dir / final_md.name
    else:
        out_json = final_json
        out_md = final_md
    cmd = [
    ...
    try:
        ...
        if atomic_replace:
            for staged, final in ((out_json, final_json), (out_md, final_md)):
                if not staged.is_file() or staged.stat().st_size == 0:
                    _die(
                        "Completed run summary generation produced no "
                        f"{staged.name}"
                    )
...
def _validate_stored_completed_verification_record(
    payload: dict[str, object],
    run_dir: Path,
    *,
    preset: str,
    ...
    """Validate the self-contained, read-only completed-run attestation.

    CLI input identity is deliberately checked by the caller because it comes
    from an independent invocation. Other consumers, including Workbench, use
    this same contract for timestamp, command, checks, log, verifier payload,
    and artifact seals without needing the original transient upload.
    """
    ...
    if not isinstance(command, list):
        _die("Completed run verification record command must be a non-empty list")
    try:
        metadata_index = command.index("--target-metadata")
        target_metadata = Path(command[metadata_index + 1])
    except (ValueError, IndexError, TypeError):
        _die("Completed run verification record command is missing target metadata")
    args = argparse.Namespace(
        preset=preset,
        mode=mode,
        target_metadata=target_metadata,
        allow_safety_degraded="--allow-degraded" in command,
    )
    recorded_metadata = payload.get("target_metadata")
    if recorded_metadata is not None:
        if (
            not isinstance(recorded_metadata, dict)
            or recorded_metadata.get("path") != str(target_metadata)
            or recorded_metadata.get("sha256")
            != _target_metadata_sha256(target_metadata)
        ):
            _die(
                "Completed run verification record target metadata identity "
    ...
    _validate_completed_verified_artifacts(payload, run_dir, preset)
...
def _read_completed_verification_record(
    ...
    # The shared validator derives target metadata from the sealed command so
    # non-CLI consumers can validate a self-contained record. A CLI re-read has
    # an independent requested metadata path and must bind that request too.
    _validate_completed_verification_command(args, payload, run_dir)
...
def _verify_existing_run(args: argparse.Namespace) -> int:
    ...
    run_dir = _run_dir(args)
    manifest_path = run_dir / RUN_MANIFEST_JSON
    has_recorded_identity = manifest_path.exists()
    if has_recorded_identity:
        # Validate the recorded identity before any regeneration touches the
        # completed contract. A corrupt or content-mismatched manifest must
        # never be "repaired" by writing a fresh summary over the old bytes.
        _ensure_run_manifest(args)
    # Only a run with a recorded, validated identity has prior bytes whose
    # preservation the contract promises; legacy migration regenerates in place.
    _run_output_summary(args, atomic_replace=has_recorded_identity)
...
def build_snakemake_command(
    ...
    *,
    ...
    # SKINSCOUT_RESULTS_ROOT로 실행 루트를 바꾸면, 이 스크립트는 그 경로에서
    # run_manifest·검증을 읽는데 Snakemake는 config.yaml의 results/runs에 써서
    # 마지막 단계에서 서로 다른 디렉터리를 보게 된다. 환경변수가 있고 사용자가
    # --extra-config로 명시하지 않았다면 점 표기 그대로 전달한다(반드시
    # _snakemake_config_entries를 거쳐야 paths.* 중첩 항목으로 합쳐진다).
    # _effective_workflow_config()가 같은 목록을 사용하므로 readiness와
    # verifier도 동일한 경로를 본다.
    config.extend(_snakemake_config_entries(_workflow_config_override_values(args)))
    if args.allow_safety_degraded:
...
def main(argv: list[str] | None = None) -> int:
    ...
    effective_config = _effective_workflow_config(args)
    _register_active_stage0_config(effective_config)
