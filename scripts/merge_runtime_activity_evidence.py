...
def _project(frame: pd.DataFrame, prefix: str) -> pd.DataFrame:
    ...
    projected = pd.DataFrame(
        {
            "molecule_chembl_id": prefix + ":" + frame["ligand_id"].astype(str).str.strip(),
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "licences": sorted(set(extra["source_license"].astype(str))),
        "policy": {
            ...
            "ligand_key_namespaces": {"BDB": "BindingDB ligand id", "GTP": "GtoPdb ligand id"},
        },
    }
    for path, label in ((args.bindingdb, "bindingdb"), (args.gtopdb, "gtopdb")):
        if path.exists() and any(label in tier for tier in tiers):
            manifest["inputs"][label] = {"path": str(path.resolve()), "sha256": _sha256(path)}
