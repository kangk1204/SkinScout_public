...
from run_skinscout import (  # noqa: E402
    ...
    model_readiness_command,
    validate_discovery_alias_package,
)
from data_readiness import resolve_stage0_paths  # noqa: E402
...
def _model_group(
    *,
    ...
    cmd = model_readiness_command(required, readiness_command, use_conda=use_conda)
    res = subprocess.run(
        cmd,
        cwd=ROOT,
        text=True,
...
def _stage0_claim_quality_group(
    *,
    repo_root: Path,
    paths: Mapping[str, Path] | None = None,
    pdbqt_policy: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    checks = stage0_verify.collect_checks(
        ...
        paths=paths,
        pdbqt_min_success_fraction=(
            pdbqt_policy.get("min_success_fraction")
            if pdbqt_policy is not None
            else None
        ),
        pdbqt_min_success_count=(
            pdbqt_policy.get("min_success_count")
            if pdbqt_policy is not None
            else None
        ),
    )
...
def readiness_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    workflow_config: Mapping[str, Any] | None = None
    try:
        workflow_config = _effective_workflow_config(_runner_namespace(args))
    except SystemExit:
        workflow_config = None
    stage0_paths = (
        resolve_stage0_paths(workflow_config, root=args.repo_root)
    ...
    if (
        ...
        groups.append(
            _stage0_claim_quality_group(
                repo_root=args.repo_root,
                paths=stage0_paths,
                pdbqt_policy=(
                    workflow_config.get("stage0")
                    if workflow_config is not None
                    else None
                ),
            )
