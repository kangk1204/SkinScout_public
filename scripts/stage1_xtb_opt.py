...
def main() -> None:
    ...
    if not shutil.which("xtb"):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Dimorphite가 고른 양성자 상태는 중성이 아닐 수 있다(예: 카복실산 음이온).
            # xTB에 --chrg를 주지 않으면 기본값 0으로 계산되어 전자 수가 틀어진
            # 전자구조·기하가 조용히 만들어진다(실측: 비중성 선택 33/54).
            charge = Chem.GetFormalCharge(base_mol)
            unpaired = Descriptors.NumRadicalElectrons(base_mol)
            cmd = ["xtb", str(xyz), f"--gfn{args.gfn}", "--opt", "tight",
                   "--alpb", "water", "--chrg", str(charge)]
            if unpaired:
                cmd += ["--uhf", str(unpaired)]
            ...
            try:
                ...
                ok = res.returncode == 0 and read_xyz_back(base_mol, opt_xyz)
                fail_tail = (res.stderr or res.stdout or "invalid_xtbopt_xyz")[-500:]
            except subprocess.TimeoutExpired:
                xtb_returncode = None
                ok = False
                fail_tail = "xtb_timeout_1800s"
            if ok:
                pass
            else:
                xtb_error_tail = fail_tail
                if not args.allow_mmff_fallback:
                    raise SystemExit(
