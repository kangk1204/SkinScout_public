...
selected receptor is prepared with run-specific AutoGrid4 map/field coverage (`.maps.fld` or
`.fld`). The workflow generates those maps and verifies their manifest before docking.
...
## Stage 7 / 7.5 / 8 — MD, Retrosynthesis, QM

Stage 7 is enabled only for lineage-verified bound complex poses and performs
protein+ligand topology merge, solvation, charge neutralisation with 0.15 M
background salt, minimization, position-restrained NVT/NPT equilibration
(`define = -DPOSRES` against the `grompp -r` reference), and production MD.
The MM-GBSA values reported from those trajectories are the entropy-free GB
`DELTA TOTAL` evaluated at `saltcon = 0.150` M — an interaction enthalpy, not
an entropy-corrected binding free energy. Retrosynthesis
