...
def render_block(state: dict) -> str:
    ...
    lines = [
        ...
        f"runtime decision `{runtime.get('promotion_decision')}`, "
        f"`claim_ready={str(runtime.get('claim_ready')).lower()}` — 런타임 인덱스는 동결 "
        "평가 인덱스가 아니므로 동결 평가 수치를 운영 성능 주장으로 쓸 수 없다.",
        f"- 같은 게이트의 evaluation decision: `{evaluation.get('promotion_decision')}`, "
        ...
        f"{known_selected['pairs']}쌍 known panel: baseline "
        ...
        f"- temporal 2025: baseline Top10 {temporal_baseline['top10']}/{temporal_baseline['pairs']} · "
