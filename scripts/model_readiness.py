...
from stage3_gnina_rescore import gnina_launch_error, resolve_gnina_binary
...
# Upstream loaders import these at model load time. Under
# --active-environment-only readiness probes their presence with find_spec()
# (cheap) instead of trusting the bundled adapter's importability, which says
# nothing about the interpreter that will actually run inference.
UPSTREAM_RUNTIME_DEPENDENCIES = {
    "rtmscore": ("torch", "dgl"),
    "psichic": ("torch", "torch_geometric"),
}
UPSTREAM_CHECKPOINT_FILES = {
    "psichic": ("config.json", "degree.pt", "model.pt"),
}
...
def gnina_status() -> dict[str, object]:
    if os.environ.get("SKINSCOUT_DISABLE_LOCAL_TOOL_PATHS") == "1":
        return {"status": "missing", "path": None}
    try:
        binary = resolve_gnina_binary(os.environ.get("SKINSCOUT_GNINA_BINARY"))
        if binary is None:
            return {"status": "missing", "path": None}
        error = gnina_launch_error(
            binary,
            os.environ.get("SKINSCOUT_GNINA_RUNTIME_PREFIX"),
        )
    except SystemExit as exc:
        return {"status": "missing", "path": None, "reason": str(exc)}
    if error is not None:
        return {
            "status": "missing",
            "path": binary,
            "reason": f"GNINA --help launch failed: {error}",
        }
    return {"status": "available", "path": binary, "probe": "--help; LD_BIND_NOW=1"}
...
def missing_runtime_modules(modules: tuple[str, ...]) -> list[str]:
    return [
        module for module in modules if importlib.util.find_spec(module) is None
    ]


def checkpoint_read_error(path: Path) -> str | None:
    """Read one byte so permission and truncated-file failures surface early."""
    if not path.is_file():
        return f"missing checkpoint: {path}"
    if path.stat().st_size <= 0:
        return f"empty checkpoint: {path}"
    try:
        with path.open("rb") as handle:
            handle.read(1)
    except OSError as exc:
        return f"unreadable checkpoint: {path}: {exc}"
    return None
...
def rtmscore_status(*, probe_runtime: bool = False) -> dict[str, object]:
    ...
    if missing:
        ...
        return status
    if probe_runtime:
        status["probe"] = "runtime_dependencies;checkpoint_readability"
        missing_dependencies = missing_runtime_modules(
            UPSTREAM_RUNTIME_DEPENDENCIES["rtmscore"]
        )
        if missing_dependencies:
            status.update(
                {
                    "status": "missing",
                    "reason": "missing RTMScore runtime dependencies",
                    "missing_dependencies": missing_dependencies,
                }
            )
            return status
        read_error = checkpoint_read_error(model)
        if read_error is not None:
            status.update(
                {
                    "status": "missing",
                    "reason": "unreadable RTMScore checkpoint",
                    "unreadable_checkpoints": [read_error],
                }
            )
    return status


def psichic_status(*, probe_runtime: bool = False) -> dict[str, object]:
    ...
    if probe_runtime:
        status["probe"] = "runtime_dependencies;checkpoint_readability"
        missing_dependencies = missing_runtime_modules(
            UPSTREAM_RUNTIME_DEPENDENCIES["psichic"]
        )
        if missing_dependencies:
            status.update(
                {
                    "status": "missing",
                    "reason": "missing PSICHIC runtime dependencies",
                    "missing_dependencies": missing_dependencies,
                }
            )
            return status
        weight_dir = root / "trained_weights" / model_name
        unreadable = [
            detail
            for detail in (
                checkpoint_read_error(weight_dir / filename)
                for filename in UPSTREAM_CHECKPOINT_FILES["psichic"]
            )
        ...
        if unreadable:
            status.update(
                {
                    "status": "missing",
                    "reason": "unreadable PSICHIC checkpoint",
    ...
    payload: dict[str, object] = {
        ...
        "imports": {
            "rtmscore": rtmscore_status(
                probe_runtime=args.active_environment_only,
            ),
