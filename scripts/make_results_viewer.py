    ...
    for row in rows:
        ...
        if has_evidence:
            ...
            extra_cells += (
                ...
                f'<td class="num" data-value="{row.get("nearest") or ""}">'
                f'{_fmt(row.get("nearest"))}</td>'
            )
        cells = "".join(
        ...
        body_rows.append(
            ...
            f'<td class="num" data-value="{row["consensus"] or ""}">{_fmt(row["consensus"], 5)}</td>'
            f'<td class="num" data-value="{row["source_count"] or ""}">{row["source_count"]}</td>'
...
<p class="sub">{html.escape(row['protein'])} · 합의 순위 {row['rank']}위 ·
   점수를 낸 방법 {row['source_count']}개 ({html.escape(row['sources'])})</p>
{warn_block}
{evidence_block}
<h2>점수</h2>
