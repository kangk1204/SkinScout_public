...
_BLANK_VALUES = {"", "nan", "none", "<na>", "null"}


def _clean_metadata_values(series: pd.Series) -> list[str]:
    text = series.fillna("").map(lambda value: str(value).strip())
    return sorted({value for value in text if value.lower() not in _BLANK_VALUES})


def _evidence_manifest_path(evidence_dir: Path) -> Path | None:
    """The manifest that documents where each source's rows came from."""
...
def _evidence_licensing(evidence_dir: Path, projected: pd.DataFrame) -> dict[str, Any]:
    """Carry the evidence table's per-source release/licence lineage onto the index.
    ...
    a path someone has to remember to check. The record is per actual source, not
    a single union: a ChEMBL row with no ChEMBL licence metadata used to pass
    because only the added tier's licence was written down.
    """
    ...
    if manifest.is_file() and manifest.stat().st_size > 0:
        ...
        manifest_path = _evidence_manifest_path(evidence_dir)
        sources_tier = "chembl-only"
        sources = ["chembl"]
    if manifest_path is None:
        raise SystemExit(
            f"evidence directory {evidence_dir} has no manifest (manifest.json or "
            "source_manifest.json); cannot record per-source lineage"
        )
    ...
    empty = pd.Series("", index=projected.index, dtype=object)
    for source_db, group in projected.groupby("source_db", sort=True):
        source = str(source_db).strip()
        releases = _clean_metadata_values(
            group.get("source_release", empty.reindex(group.index))
        )
        licences = _clean_metadata_values(
            group.get("source_license", empty.reindex(group.index))
        )
        if not releases:
            raise SystemExit(
                f"runtime evidence source {source!r} has no release metadata; "
                "rebuild the evidence table before building the index"
            )
        if not licences:
            raise SystemExit(
                f"runtime evidence source {source!r} has no licence metadata; "
                "rebuild the evidence table before building the index"
            )
    ...
    if not source_metadata:
        raise SystemExit("runtime evidence projection has no source rows to license")
    union_licences = sorted(
        {licence for record in source_metadata.values() for licence in record["licences"]}
    )
    return {
        "sources_tier": sources_tier,
        "sources": sources,
        "licences": union_licences,
...
def project_runtime_evidence(
    ...
    def _optional_text(name: str) -> pd.Series:
    ...
    projected = pd.DataFrame(
        {
            ...
            "source_release": _optional_text("source_release"),
            "source_license": _optional_text("source_license"),
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "projection": stats,
        "evidence_licensing": _evidence_licensing(args.evidence_dir, projected),
