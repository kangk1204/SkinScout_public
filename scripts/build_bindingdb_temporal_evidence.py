...
Compound identity is recorded apart from the measurement/assay identifier.
BindingDB MonomerID (with a documented fallback chain) keys the compound, the
Reactant_set/Entry id keys the measurement, and an InChIKey-based structure id
carries the standardised structure. The legacy `ligand_id` column is retained
byte-compatible but may hold a measurement id and must not key a compound.
"""
...
# Legacy `ligand_id`, kept exactly as built before so existing artifacts keep
# their meaning. It is measurement-first (Reactant_set_id), which is the bug
# F32 records; new code must key compounds on `source_compound_id` instead.
LIGAND_ID_COLUMNS = (
...
# Compound identity. BindingDB MonomerID identifies the compound; a
# Reactant_set_id is an assay/measurement-level id and must not key a compound.
SOURCE_COMPOUND_ID_COLUMNS = (
    "BindingDB MonomerID",
    "Ligand ID",
    "ChEMBL ID of Ligand",
    "Molecule ChEMBL ID",
...
# Measurement/assay identity. Reactant_set_id names the ligand record of an
# assay; Entry ID names the affinity measurement itself. Neither is a compound.
...
OUTPUT_SCHEMA = pa.schema(
    [
        ...
        ("source_compound_id_basis", pa.string()),
        ("measurement_id", pa.string()),
        ("measurement_id_basis", pa.string()),
        ("structure_id", pa.string()),
        ("structure_id_basis", pa.string()),
        ("uniprot", pa.string()),
...
def _validate_headers(fieldnames: list[str], max_chains: int) -> dict[str, object]:
    ...
    source_compound_cols = _all_cols(fieldnames, SOURCE_COMPOUND_ID_COLUMNS)
    measurement_cols = _all_cols(fieldnames, MEASUREMENT_ID_COLUMNS)
    ...
    return {
        ...
        "inchikey": inchikey,
        "ligand_id": ligand_id,
        "source_compound_cols": source_compound_cols,
...
def _first_value_with_basis(
    row: dict[str, str], cols: Iterable[str]
) -> tuple[str, str]:
    """Return the first populated value and the source column that supplied it.

    The basis is carried into the output so a downstream reader never has to
    guess which namespace an id came from.
    """
    for col in cols:
        value = _clean_text(row.get(col))
        if value:
...
def _structure_id(smiles: str, inchikey: str) -> tuple[str, str]:
    """Standardised structure identity, distinct from compound and measurement ids.

    InChIKey is the standard structure hash. When only a SMILES is present it is
    hashed verbatim and the basis says it is not stereo-standardised - calling
    that a canonical structure id would be a claim the bytes do not support.
    """
    if inchikey:
        return f"inchikey:{inchikey}", "standard_inchikey"
    if smiles:
        digest = hashlib.sha256(smiles.encode("utf-8")).hexdigest()
        return f"smiles-sha256:{digest}", "raw_smiles_sha256_not_stereo_standardised"
    return "", ""
...
def normalize_bindingdb(args: argparse.Namespace) -> dict[str, object]:
    ...
    source_compound_id_basis_counts: Counter[str] = Counter()
    measurement_id_basis_counts: Counter[str] = Counter()
    structure_id_basis_counts: Counter[str] = Counter()
    source_id_differs_from_measurement_id_rows = 0
        ...
        with iter_bindingdb_rows(args.bindingdb_tsv) as (fieldnames, reader):
            ...
            for row in reader:
                ...
                if not (ligand_smiles or ligand_inchikey or ligand_id):
                    continue
                structure_id, structure_id_basis = _structure_id(
                    ligand_smiles, ligand_inchikey
                )
                source_compound_id, source_compound_id_basis = _first_value_with_basis(
                    row, cols["source_compound_cols"]
                )
                if not source_compound_id:
                    if ligand_id:
                        source_compound_id = ligand_id
                        source_compound_id_basis = "legacy_ligand_id_fallback"
                    elif structure_id:
                        source_compound_id = structure_id
                        source_compound_id_basis = f"{structure_id_basis}_fallback"
                measurement_id, measurement_id_basis = _first_value_with_basis(
                    row, cols["measurement_cols"]
                )
                if not measurement_id:
                    measurement_id = f"input-row:{rows_seen}"
                    measurement_id_basis = "input_row_number_fallback"
                if (
                    source_compound_id
                    and measurement_id
                    and source_compound_id_basis in cols["source_compound_cols"]
                    and measurement_id_basis in cols["measurement_cols"]
                    and source_compound_id != measurement_id
                ):
                    source_id_differs_from_measurement_id_rows += 1
                source_compound_id_basis_counts[source_compound_id_basis or "missing"] += 1
                measurement_id_basis_counts[measurement_id_basis or "missing"] += 1
                structure_id_basis_counts[structure_id_basis or "missing"] += 1
                (
                    source_db,
                    source_origin,
                    source_license,
                ...
                for affinity_type, activity_col in cols["activity_cols"].items():
                    ...
                    for uniprot in uniprots:
                        identity: dict[str, object] = {
                            ...
                            "source_compound_id_basis": source_compound_id_basis,
                            "measurement_id": measurement_id,
                            "measurement_id_basis": measurement_id_basis,
                            "structure_id": structure_id,
                            "structure_id_basis": structure_id_basis,
                            "uniprot": uniprot,
    ...
    manifest = {
        ...
        # F32: measurement/assay ids and compound ids are different namespaces.
        # `ligand_id` is the legacy field and may be a measurement id; identity
        # consumers must use source_compound_id, never ligand_id.
        "id_namespaces": {
            "ligand_id": {
                "role": "legacy column retained byte-compatible; not a compound key",
                "basis": "first available of: " + ", ".join(LIGAND_ID_COLUMNS),
                "may_be_measurement_id": True,
            },
            "source_compound_id": {
                "role": "compound identity for downstream ligand keys",
                "basis": (
                    "first available of: "
                    + ", ".join(SOURCE_COMPOUND_ID_COLUMNS)
                    + "; falls back to legacy ligand_id, then structure_id"
                ),
                "is_measurement_id": False,
            },
            "measurement_id": {
                "role": "assay/measurement identity; repeated measurements of one "
                "compound share source_compound_id and differ here",
                "basis": (
                    "first available of: "
                    + ", ".join(MEASUREMENT_ID_COLUMNS)
                    + "; falls back to input_row_number"
                ),
                "is_compound_id": False,
            },
            "structure_id": {
                "role": "standardised structure identity",
                "basis": (
                    "inchikey:<Ligand InChI Key> when present; otherwise "
                    "smiles-sha256:<sha256(raw Ligand SMILES)>, which is not "
                    "stereochemistry-standardised"
                ),
            },
        },
        "counts": {
            ...
            # Input rows that produced evidence and carried both dedicated
            # columns with different values - the count the old single-column
            # design could not express.
            "source_compound_id_differs_from_measurement_id_rows": (
                source_id_differs_from_measurement_id_rows
            ),
            "source_compound_id_basis_counts": dict(source_compound_id_basis_counts),
            "measurement_id_basis_counts": dict(measurement_id_basis_counts),
            "structure_id_basis_counts": dict(structure_id_basis_counts),
        },
        "artifacts": {
            name: {
                "path": str(path),
