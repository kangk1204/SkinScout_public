...
def smiles_from_sdf(sdf: Path) -> str:
    """정규 SMILES. 명시적 수소는 떼어 다른 Stage 2 산출물과 문자열을 맞춘다.

    consensus는 다섯 입력의 SMILES가 정확히 같은지 확인한다(fail-closed).
    SDF의 수소를 그대로 쓰면 `[H]c1...`이 되어 웹 모델의 정규 SMILES와 어긋나고,
    합의 계산이 "input smiles mismatch"로 죽는다.
    """
