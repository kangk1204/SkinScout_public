...
def docking_seed(ligand: Path, target_id: str) -> int:
    """같은 (리간드, 표적)이면 항상 같은 시드.

    AutoDock-GPU의 `--seed` 기본값은 "time, process id"라, 지정하지 않으면 같은
    입력을 다시 돌려도 포즈·점수가 달라진다. 이 도구는 해시로 산출물을 묶고
    재현을 주장하므로, 리간드 파일 내용과 표적 ID에서 결정론적으로 유도한다.
    """
    digest = hashlib.sha256()
    digest.update(_safe_target_id(target_id).encode("utf-8"))
    try:
        digest.update(ligand.read_bytes())
    except OSError:
        digest.update(ligand.name.encode("utf-8"))
...
def run_autodock_gpu_one(
    ...
    cmd = [
        ...
        "--seed", str(docking_seed(ligand, target_id or map_file.stem)),
    ]
...
def main() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(dir=staging_parent) as tmp:
            ...
            if n_written == 0 and any(
                map_file is not None for map_file in map_files.values()
            ):
                # 지도는 준비됐는데(도킹 가능) 점수가 하나도 안 나온 경우다. 전부
                # 실패했는데도 0점 랭킹을 발행하면 구조 근거 없는 결과가 조용히
                # 실려 나간다(구조 없는 표적만 있는 경우와 구분해 실패시킨다).
                raise SystemExit(
                    "AutoDock-GPU produced no usable receptor scores "
                    "despite ready maps"
                )
            if (
                args.engine == "autodock_gpu"
