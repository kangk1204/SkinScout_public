#!/usr/bin/env python3
"""stage7_mmgbsa.py — gmx_MMPBSA wrapper that emits relative MM-GBSA ΔG per
target across all MD replicas.
"""
...
def run_mmgbsa(target_dir: Path, trajectories: list[Path],
               gmx_cmd: str = "gmx") -> dict | None:
    """한 표적의 MM-GBSA ΔG.
    ...
    for tool in ("gmx_MMPBSA", gmx_cmd):
        if not shutil.which(tool):
            ...
            return None
    structure = target_dir / "prod.tpr"
    topology = target_dir / "topol.top"
    for required in (structure, topology):
        if not nonempty(required):
            LOG.error("MM-GBSA input is missing or empty: %s", required)
            return None
    usable = [path for path in trajectories if nonempty(path)]
    if not usable:
        LOG.error("MM-GBSA has no non-empty trajectory in %s", target_dir)
        return None

    n_frames = trajectory_frame_count(usable[0], structure, gmx_cmd)
    input_file = write_mmpbsa_input(target_dir, n_frames)
    index_ndx = write_index_file(target_dir, structure, gmx_cmd)
    ...
    values: list[float] = []
    sems: list[float] = []
    for replica, trajectory in enumerate(usable, start=1):
...
def main() -> None:
    ...
    traj = read_trajectory_index(args.trajectory_index)
    ...
    for uid, _ in per_target:
        traj_paths = traj[traj["target_id"] == uid]
        trajectories = [Path(p) for p in traj_paths["trajectory_xtc"].astype(str)]
        if not trajectories or any(not nonempty(path) for path in trajectories):
            failed_targets.append(str(uid))
            continue
        target_dir = trajectories[0].parent
        result = run_mmgbsa(target_dir, trajectories)
        if result is None:
            failed_targets.append(str(uid))
            continue
