...
PSICHIC_STATUS_SCHEMA = "skinscout.psichic-source-status.v1"
PSICHIC_SCORE_COLUMNS = ["target_id", "psichic_score", "score"]
...
def source_status_path(out_scores: Path) -> Path:
    return out_scores.with_suffix(out_scores.suffix + ".status.json")


def _write_source_status(path: Path, payload: dict[str, object]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    status_path = source_status_path(args.out_scores)
    _remove_outputs(args.out_scores, status_path)
    ...
    try:
        import psichic as psichic_module  # type: ignore[import-untyped]
    except ImportError as exc:
        unavailable = {
            "reason": "import_error",
            "detail": str(exc),
            "missing": "psichic import",
        }
    else:
        try:
            model = psichic_module.PSICHIC.load_pretrained()
        except getattr(psichic_module, "PSICHICPrerequisiteError", ()) as exc:
            # Only the dedicated prerequisite type is degraded. Genuine
            # inference or integration failures keep their traceback.
            missing = getattr(exc, "missing", None)
            unavailable = {
                "reason": "prerequisite_missing",
                "detail": str(exc),
                "missing": missing if isinstance(missing, str) else "prerequisite",
            }
    if unavailable is not None:
        if not args.allow_unavailable:
            raise SystemExit(
                ...
                f"({unavailable['reason']}: {unavailable['detail']})"
            )
        LOG.warning(
            "psichic unavailable (%s); emitting explicit non-claimable degraded result",
            unavailable["reason"],
        )
        tmp_scores = _tmp_output(args.out_scores)
        with tmp_scores.open("w", newline="") as fh:
            csv.writer(fh, delimiter="\t").writerow(PSICHIC_SCORE_COLUMNS)
        tmp_scores.replace(args.out_scores)
        _write_source_status(
            status_path,
            {
                "schema_version": PSICHIC_STATUS_SCHEMA,
                "source": "psichic",
                "status": "unavailable",
                "claimable": False,
                "targets_scored": 0,
                "score_tsv": args.out_scores.name,
                **unavailable,
            },
        )
        return
    ...
    def cpu_fallback_model() -> object:
        ...
        if cpu_model is None:
            ...
            cpu_model = psichic_module.PSICHIC.load_pretrained(device="cpu")
        return cpu_model
    ...
    with tmp_scores.open("w", newline="") as fh:
        ...
        w.writerow(PSICHIC_SCORE_COLUMNS)
        batch: list[tuple[str, str]] = []
        def flush() -> None:
            if not batch:
                return
    ...
    _write_source_status(
        status_path,
        {
            ...
            "detail": None,
            "missing": None,
            "targets_scored": len(target_scores),
            "score_tsv": args.out_scores.name,
        },
    )
    if long_targets:
