...
try:
    from stage11_repro_pack import (
        CODE_SNAPSHOT_SCHEMA,
        EFFECTIVE_CONFIG_SCHEMA,
    )
except ModuleNotFoundError:  # pragma: no cover - importlib-based unit tests
    from scripts.stage11_repro_pack import (
        CODE_SNAPSHOT_SCHEMA,
        EFFECTIVE_CONFIG_SCHEMA,
    )
...
def _caption_claim_blockers(payload: Any, source: str) -> list[dict[str, Any]]:
    """Require measured values, units, and source hashes for claimed figures.

    A figure caption may only be labelled source_backed / claim_eligible when
    it plots a real measured quantity.  Artifact-count diagnostics therefore
    cannot be relabelled into a claim-quality figure package.
    """
    blockers: list[dict[str, Any]] = []
    ...
    for name, entry in payload.items():
        if not isinstance(entry, dict):
            continue
        claimed = (
            entry.get("status") == "source_backed"
            or entry.get("claim_eligible") is True
        )
        if not claimed:
            continue
        if (
            entry.get("status") != "source_backed"
            or entry.get("claim_eligible") is not True
            or entry.get("diagnostic_only") is not False
        ):
            blockers.append({
                "code": "source_backed_caption_inconsistent_claim_flags",
                "source": source,
                "figure": str(name),
            })
            continue
        ...
        if not isinstance(sources, list) or not sources:
            blockers.append({
                "code": "source_backed_caption_missing_measured_evidence",
                "source": source,
                "figure": str(name),
                "detail": "no sources recorded",
            })
            continue
        for idx, item in enumerate(sources):
            if not isinstance(item, dict):
                blockers.append({
                    ...
                    "source": source,
                    "figure": str(name),
                    "detail": f"source index {idx} is not an object",
                })
                continue
            ...
            if (
                ...
                blockers.append({
                    ...
                    "source": source,
                    "figure": str(name),
                    "detail": f"source index {idx} lacks value/unit/sha256",
                })
    return blockers
...
def _valid_limitation(value: object) -> bool:
    return bool(
        ...
        and value["approved_by"].strip()
    )


def _bound_repro_sidecar(
    manifest_path: Path,
...
def _repro_provenance(
    ...
    provenance["git_commit"] = values.get("git_commit.txt")
    provenance["config_hash"] = values.get("config_hash.txt")

    code_snapshot = _bound_repro_sidecar(
        manifest_path, metadata, "code_snapshot.json", blockers
    )
    if code_snapshot is not None:
        if (
            code_snapshot.get("schema_version") != CODE_SNAPSHOT_SCHEMA
            or not isinstance(code_snapshot.get("dirty"), bool)
        ):
            blockers.append({
                "code": "invalid_code_snapshot",
                "path": str(manifest_path.parent / "code_snapshot.json"),
            })
        else:
            provenance["code_snapshot"] = code_snapshot
            limitation = code_snapshot.get("limitation")
            if limitation is not None and not _valid_limitation(limitation):
                blockers.append({
                    "code": "invalid_code_snapshot_limitation",
                    "path": str(manifest_path.parent / "code_snapshot.json"),
                })
            if code_snapshot["dirty"] and not _valid_limitation(limitation):
                blockers.append({
                    "code": "dirty_code_snapshot_without_limitation",
                    "detail": (
                        "the reproducibility pack was generated from a dirty "
                        "git worktree with no recorded limitation"
                    ),
                })
            snapshot_commit = str(code_snapshot.get("git_commit", "")).strip().lower()
            if not _valid_hex(snapshot_commit, GIT_COMMIT_LENGTH):
                blockers.append({
                    "code": "invalid_code_snapshot",
                    "path": str(manifest_path.parent / "code_snapshot.json"),
                })
            elif (
                provenance["git_commit"] is not None
                and snapshot_commit != provenance["git_commit"]
            ):
                blockers.append({
                    "code": "code_snapshot_commit_mismatch",
                    "detail": (
                        "code_snapshot.json and git_commit.txt disagree on the "
                        "sealed commit"
                    ),
                })

    effective_config = _bound_repro_sidecar(
        manifest_path, metadata, "effective_config.json", blockers
    )
    if effective_config is not None:
        if effective_config.get("schema_version") != EFFECTIVE_CONFIG_SCHEMA:
            blockers.append({
                "code": "invalid_effective_config",
                "path": str(manifest_path.parent / "effective_config.json"),
            })
        else:
            provenance["effective_config"] = effective_config
            effective_digest = str(
                effective_config.get("effective_config_sha256", "")
            ).strip().lower()
            base_digest = str(
                effective_config.get("base_config_sha256", "")
            ).strip().lower()
            overrides = effective_config.get("overrides")
            if not _valid_hex(effective_digest, SHA256_LENGTH) or not _valid_hex(
                base_digest, SHA256_LENGTH
            ) or not isinstance(overrides, list) or any(
                not isinstance(override, str) for override in overrides
            ):
                blockers.append({
                    "code": "invalid_effective_config",
                    "path": str(manifest_path.parent / "effective_config.json"),
                ...
                provenance["config_hash"] is not None
                and base_digest != provenance["config_hash"]
            ):
                blockers.append({
                    "code": "effective_config_base_mismatch",
                    "detail": (
                        "effective_config.json base_config_sha256 does not match "
                        "the bound config_hash.txt"
                    ),
                })
...
def _collect_manifest(
    ...
    for spec in args.artifact:
        ...
        if status_payload is not None:
            ...
            if label == "figure_captions":
                blockers.extend(_caption_claim_blockers(status_payload, label))
    ...
    if repro_path is not None:
        repro_provenance, repro_blockers = _repro_provenance(repro_path)
        ...
        if not isinstance(eval_provenance, dict):
            ...
            if not _valid_hex(eval_git, GIT_COMMIT_LENGTH) or not _valid_hex(
                ...
                repro_provenance["git_commit"] != eval_git
                or repro_provenance["config_hash"] != eval_config
            ):
        ...
        repro_provenance["git_commit"] = sidecar_git_commit
        repro_provenance["config_hash"] = sidecar_config_hash
    ...
    payload: dict[str, Any] = {
        "run_id": run_dir.name,
        "git_commit": repro_provenance["git_commit"] or _git_commit_from_repo(),
        "config_hash": repro_provenance["config_hash"],
        "code_snapshot": repro_provenance["code_snapshot"],
        "effective_config": repro_provenance["effective_config"],
