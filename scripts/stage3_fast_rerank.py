...
def main() -> None:
    ...
    autodock = autodock.sort_values(["neg_vina_score", "target_id"],
    ...
    def to_ranked(rows: list[tuple[str, float]]) -> list[str]:
        return [u for u, _ in sorted(rows, key=lambda x: (-x[1], x[0]))]
    ...
    df = df.sort_values(["rrf_score", "target_id"], ascending=[False, True],
                            kind="mergesort").head(args.top_n)
