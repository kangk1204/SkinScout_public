...
def rank_targets(scores: np.ndarray, scorable: np.ndarray) -> np.ndarray:
    """Average-tie ranks, with targets the model cannot score left unscored.

    `np.argsort` gives all 20,204 targets distinct ordinal ranks. A truth
    target with no activity edge reads 0.0 and therefore lands wherever the
    target array happens to put it - with fewer than 30 positive targets that
    can be a Top30 "hit" for a target nothing retrieved. Unscorable targets get
    `inf` here so the band table cannot count them, and exact-score ties get
    the shared average rank, which is invariant under target-array permutation.
    """
    from activity_retrieval_scoring import average_tie_ranks

    ranks = average_tie_ranks(scores)
    mask = np.asarray(scorable, dtype=bool)
    if mask.shape != ranks.shape:
        raise ValueError("scorable mask and ranking scores must have equal shape")
    return np.where(mask, ranks, np.inf)
...
def main() -> int:
    ...
    for record in panel.itertuples(index=False):
        ...
        ranks = rank_targets(scores, scorable)
        ...
        for target in (t.strip() for t in str(record.known_targets).split(";") if t.strip()):
            ...
            rows.append({
                ...
                # A target with no activity edge was never scored; its zero
                # score is the tail, not a retrieved rank.
                "in_scorable_universe": bool(scorable[index]),
            })
    ...
    for low, name in BANDS:
        ...
        scored = band.loc[band["in_scorable_universe"], "rank"]
