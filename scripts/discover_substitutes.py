...
try:
    from .compound_applicability import physchem
except ImportError:  # Executed as a standalone script.
    from compound_applicability import physchem  # type: ignore[no-redef]
...
def _mcs_atom_map(parent: Chem.Mol, candidate: Chem.Mol) -> list[tuple[int, int]]:
    ...
    if not parent_match or not candidate_match:
        return []
    return [(int(candidate_atom), int(parent_atom)) for parent_atom, candidate_atom in zip(parent_match, candidate_match, strict=True)]
...
def _pharmacophore_3d_comparison(
    ...
    *,
    ...
    atom_map = _mcs_atom_map(parent_mol, candidate_mol)
    if len(atom_map) < 3:
        return {
            "status": "unavailable",
            "basis": "etkdg_v3_mmff_ligand_feature_alignment",
    ...
    return {
        ...
        "feature_distance_rmsd": _round(best_rmsd),
    }
...
def _write_sdf(path: Path, parent: dict[str, Any], rows: list[dict[str, Any]], seed: int) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_name(f".{path.name}.tmp-{os.getpid()}")
