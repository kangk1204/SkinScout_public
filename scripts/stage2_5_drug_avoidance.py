...
def fingerprint_input_mols(mol: Chem.Mol) -> list[Chem.Mol]:
    """이온형(Stage 1 미세상태)과 중성 표준화형을 함께 돌려준다.

    승인 성분 참조(drugs parquet)는 원본 ChEMBL SMILES에서 만들어져 이온형·중성형이
    섞여 있다. 한쪽 질의만 쓰면 반대쪽 참조와 유사도가 임계값 아래로 떨어져
    STRICT/SOFT 경고를 놓친다(실측: 이온 참조가 중성 질의에 0.5로 하락). 두 질의 중
    큰 유사도를 쓰면 어느 표현으로 저장됐든 같은 분자를 매칭할 수 있다.
    """
    ...
    queries = [mol]
    try:
        neutral = standardize_parent(mol)
    except ValueError:
        neutral = None
    if neutral is not None and Chem.MolToSmiles(neutral) != Chem.MolToSmiles(mol):
        queries.append(neutral)
    return queries
...
def main() -> None:
    ...
    query_mols = fingerprint_input_mols(mol)
    query_bvs = [MORGAN_GENERATOR.GetFingerprint(query) for query in query_mols]
    query_scaffolds = {
        Chem.MolToSmiles(MurckoScaffold.GetScaffoldForMol(query))
        for query in query_mols
    }
    query_scaffolds.discard("")
    ...
    if args.drugs_parquet.exists():
        ...
        for _, row in drugs.iterrows():
            ...
            sim = max(
                DataStructs.TanimotoSimilarity(query_bv, bv) for query_bv in query_bvs
            )
    ...
    if args.scaffolds_parquet.exists():
        ...
        matched_scaffolds = sorted(query_scaffolds & valid_scaffolds)
        if matched_scaffolds:
            warnings.append({"tier": "SCAFFOLD_MATCH",
                             "scaffold_smiles": matched_scaffolds[0]})
    else:
        reference_status = "missing_degraded"
