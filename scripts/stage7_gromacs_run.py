...
def replica_tpr(target_dir: Path, start_gro: Path,
                base_mdp: Path, topology: Path, replica: int, seed: int,
...
def mdrun(
    ...
    if xtc.exists():
        xtc.unlink()
    nsteps = int(duration_ns * 1_000_000 / timestep_fs)
    cmd = [
        gmx_cmd, "mdrun",
        "-s", str(tpr),
...
def read_md_manifest(path: Path) -> pd.DataFrame:
    ...
    lineage_paths = {"npt_gro", "prod_mdp"}
    missing_lineage = sorted(lineage_paths - set(manifest.columns))
    if missing_lineage:
        raise SystemExit(f"MD manifest missing required columns {missing_lineage}")
    for col in sorted(lineage_paths):
        invalid = manifest[col].isna() | (manifest[col].astype(str).str.strip() == "")
        if invalid.any():
            raise SystemExit(f"MD manifest column '{col}' contains blank values")
        manifest[col] = manifest[col].astype(str).str.strip()
    return manifest
...
def validate_ready_files(manifest: pd.DataFrame) -> None:
    provenance = {
        "system_gro": "system_gro_sha256",
        "topology_top": "topology_top_sha256",
        "tpr": "tpr_sha256",
        "npt_gro": "npt_gro_sha256",
        "prod_mdp": "prod_mdp_sha256",
    }
    missing_cols = sorted(
        {column for pair in provenance.items() for column in pair} - set(manifest.columns)
    )
    if missing_cols:
        raise SystemExit(f"MD manifest missing required provenance columns {missing_cols}")
    for _, row in manifest.iterrows():
        uid = str(row["target_id"])
        for path_col, hash_col in provenance.items():
            path = Path(str(row[path_col]))
            if not _nonempty(path):
                raise SystemExit(
                    f"Ready MD system file missing or empty for {uid}: "
                    f"{path_col}={path}"
                )
            expected = str(row[hash_col]).strip().lower()
            if not SHA256_RE.fullmatch(expected):
                raise SystemExit(
                    f"MD manifest {hash_col} for {uid} must be a SHA-256 digest"
                )
            actual = sha256_file(path)
            if actual != expected:
                raise SystemExit(
                    f"MD manifest {path_col} sha256 mismatch for {uid}: "
...
def main() -> None:
    ...
    if args.timestep_fs not in (1.0, 2.0):
        raise SystemExit(f"--timestep-fs must be 1.0 or 2.0: {args.timestep_fs!r}")
    if args.replicas < 1:
    ...
    try:
        for _, row in manifest.iterrows():
            ...
            tpr = Path(row["tpr"])
            npt_gro = Path(row["npt_gro"])
            prod_mdp = Path(row["prod_mdp"])
            topology_top = Path(row["topology_top"])
            target_dir = system_gro.parent
            for rep in range(1, args.replicas + 1):
                # Recheck immediately before the external tool consumes the
                # preparation outputs. Long preceding replicas must not leave
                # a window where changed inputs are silently accepted.
                validate_ready_files(pd.DataFrame([row]))
                seed = args.seed + rep - 1
                # 복제마다 자기 tpr 을 만든다. 같은 tpr 을 나눠 쓰면 시작 속도와
                # 열욕 씨앗이 같아, 두 실행의 차이가 GPU 수치 잡음뿐이 된다.
                replica_input = replica_tpr(
                    target_dir, npt_gro, prod_mdp, topology_top,
                    rep, seed, gmx_cmd,
                )
                if replica_input is None:
                    ...
                    rows.append([
                        ...
                        str(tpr), sha256_file(tpr), str(topology_top),
                        sha256_file(topology_top), seed, args.duration_ns,
                        int(args.duration_ns * 1_000_000 / args.timestep_fs),
                    ])
                    continue
                ok = mdrun(
                    target_dir, replica_input, args.duration_ns, rep, seed, gmx_cmd,
                    args.timestep_fs,
                )
                trajectory = target_dir / f"prod_r{rep}.xtc"
                if ok:
