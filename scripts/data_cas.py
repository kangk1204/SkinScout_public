...
def download_entry(
    ...
    *,
    ...
    for url in _file_sources(entry, mirror):
        ...
        try:
            with urllib.request.urlopen(request, timeout=60) as response, _open_partial_append_nofollow(partial) as out:
                ...
                if resume_from:
                    ...
                    if status == 200:
                        ...
                        return download_entry(entry, staging=staging, mirror=mirror)
                    if status != 206 or not content_range or not content_range.startswith(expected_prefix):
                        raise CasError(
                            f"resume response for {entry['path']} must be 206 with matching Content-Range"
                        )
