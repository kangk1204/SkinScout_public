...
**순위는 ③에서 정해집니다.** 도킹은 11~50위 구간만 손대고, 주석은 순서를 건드리지
않습니다. 어느 줄이 무엇 때문에 그 자리에 있는지는 결과 표의 `순위 기준` 열에 적힙니다.
...
표 위에 적습니다. `정렬`은 아홉 가지(종합 점수·핵심구조·합친 순위·유사도·안전·활성 근거·피부반응·logP·극성)이고, 그중 여섯은 검색을 다시 하지 않고 즉시 바뀝니다. 두 신호를 왜 따로 두는지, 등급이 어떤 조건에서 갈리는지, 계산이 어디서 멈추는지
는 [`docs/ALTERNATIVES.md`](./docs/ALTERNATIVES.md)에 있습니다.
...
> 다만 해시가 맞는다는 것은 받은 파일이 온전하다는 뜻입니다 — **깨끗한 Ubuntu에서 설치부터 전체 분석까지 검증된 배포판을 뜻하지는 않습니다.** 이 설치가 끝까지 확인된 것은 개발용 호스트(RTX 3080 Ti) 한 대뿐이라, 처음 까는 자리에서는 드라이버·디스크·네트워크로 멈출 수 있고 그때는 **Linux·GPU 담당자가 필요합니다.** 실험만 하실 분은 아래 1-B나, 담당자가 띄워 둔 서버에 브라우저로 붙는 쪽(§9)이 훨씬 빠릅니다.
...
번들(약 115 MB)에는 등재 원료 구조, ADMET 예측, 활성 측정 인덱스가 들어 있습니다.
저장소에 없고 이 설치가 만들지도 않습니다 — 만들려면 ChEMBL·BindingDB 원본
55 GB가 필요합니다. 담당자가 `python scripts/build_analog_bundle.py` 로 만듭니다.
이 설치로는 표적 예측·도킹·MD가 돌지 않습니다.

이미 받아둔 건 다시 받지 않습니다. 디스크를 지우거나 부팅 설정을 건드리지 않고,
분석 파일은 전부 이 컴퓨터 안에만 저장됩니다.

GPU 드라이버는 건드리지 않습니다. `nvidia-smi`가 안 되면 드라이버를 먼저 깔고
같은 명령을 다시 넣어주세요.
...
**후보 분자 3D**는 대체소재 표의 `3D` 버튼으로 바로 뜹니다(분석도 표적도 필요 없습니다). 다만 **계산으로 만든 컨포머 하나**이고 실험 구조도 결합 자세도 아닙니다. **표적 단백질 3D**는 분석이 끝나면 알아서 같이 만들어집니다. 보는 방법은 둘입니다.
...
| 유사도만으로 매긴 순위 (재정렬 전) | `03_targets/mode_fast/daina_structural_targets.csv` |
...
**방법 B — 네트워크에 직접 열기 (로그인이 필요합니다)**

터널 없이 여러 명이 같이 쓰시려면 서버에서 이렇게 켭니다.

```bash
python3 scripts/run_workbench.py --host 0.0.0.0 --port 8080
```

이렇게 밖으로 여는 순간 **로그인이 자동으로 켜집니다.** 켤 때 화면에 접속 토큰이
한 번 뜨고, 같은 값이 이 파일에도 저장됩니다.
...
> **로그인을 끈 채로는 네트워크에 열리지 않습니다.** 실수로 아무나 들어올 수 있게
> 되는 걸 막으려고 서버가 아예 시작을 거부합니다.
>
> 다만 **통신이 암호화되지는 않습니다**(평문 HTTP). 믿을 수 있는 사내망에서만 쓰시고,
> 인터넷에 열어야 한다면 앞에 HTTPS 리버스 프록시를 두세요.
...
Given a small-molecule SMILES, produce a ranked list of likely human protein
targets and an integrated Mol\* HTML report. The public Demo path is:
...
The retrieval order is what the ranking is. Band re-ranking touches only
positions 11-50; the top 10 keep the retrieval order. Structural,
skin-expression, KG, and known-target evidence annotate that order and never
silently rerank it, and every row carries a `ranking_basis` naming the column
its position actually came from.

Each target also reports what its nearest analogue's own measurement said - at,
between, or below the index's activity thresholds. The score weights similarity
far more than potency, so that label is how a reader tells a potent analogue
from a weak one. Full/Advanced adds PSICHIC, RTMScore, Boltz-2, BioEmu, MD and
QM workflows with their own fail-closed readiness and claim boundaries.

Docking is not limited to skin proteins. It runs over the entire human proteome
(20,504 raw AlphaFold v4 F1 representatives; 23,391 model files; 20,171 cleaned
unique receptors), and skin information enters as a 30 % final-score weight plus
skin-efficacy KG labels for top hits. Orphan and non-skin-enriched targets are
...
|---|---|---|---|
| GPU | 16 GB | **12 GB (RTX 3080 Ti)** | Pocket-centric crop (±20 Å), `boltz2_max_residues=700` cap; 실행 전 active compute process 0개 / 6,144 MiB free / 90% 이하 utilization admission |
...
## License policy (TL;DR)

Every dependency in `INSTRUCTIONS.md §1` has been verified commercial-use
permissive. The full audit lives in
[`LICENSE_POLICY.md`](./LICENSE_POLICY.md). Excluded: AlphaFold3 weights,
Chai-1 weights, Schrödinger Glide/FEP+, Gaussian, AMBER `pmemd.cuda`, DEREK
Nexus, ChemTunes. AlphaFold DB itself is CC-BY-4.0 — attribution required.
