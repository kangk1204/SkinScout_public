(() => {
  ...
  const VALID_VIEWS = new Set(["setup", "analyze", "alternatives", "targets", "runs", "results", "explore", "settings"]);
  ...
  async function api(path, options = {}) {
    ...
    const response0 = await fetch(path, {
      ...requestOptions,
    ...
    if (response.status === 403 && method !== "GET") {
      // 서버가 CSRF 토큰을 회전하면(1시간) 다음 POST가 403이 된다. 상태를 새로
      // 받아 토큰을 갱신하고 한 번만 재시도한다.
      try {
        await loadStatus(true);
      } catch (_) { /* 상태 갱신 실패 시 원래 403을 그대로 보고한다 */ }
      const retryToken = state.status?.csrf_token;
      if (retryToken) {
        response = await fetch(path, {
          ...requestOptions,
          headers: { "Content-Type": "application/json", "X-SkinScout-Token": retryToken, ...optionHeaders },
        });
  ...
  function statusLabel(status) {
    const labels = { ready: "준비됨", warning: "주의", partial: "부분 준비", action: "작업 필요", blocked: "차단됨", manual: "수동 준비", available: "가능", unregistered: "등록 필요", missing: "없음", queued: "대기 중", running: "실행 중", cancel_requested: "취소 중", cancelled: "취소됨", completed: "완료", failed: "실패", incomplete: "미완료", stale: "오래된 상태", verification_failed: "검증 실패", unverified: "미검증", untrusted_diagnostic: "불신 진단" };
...
function skinTierLabel(tier) {
  ...
  function reasonLabel(reason) {
    ...
    const exact = {
      ...
      "skin efficacy evidence is automatic literature co-occurrence and requires review": "피부 효능 근거가 자동 문헌 공출현이라 사람이 검토해야 합니다.",
  ...
  function navigate(view) {
    ...
    if (view === "targets") renderTargetsReadiness();
    ...
    list.innerHTML = visibleRuns.map((run) => {
      ...
      const verificationNote = run.status === "verification_failed"
        ...
        : run.status === "unverified"
          ...
          : "";
      const outputsNote = run.outputs_available === false && !running
        ? `<p class="muted">산출물이 남아 있지 않습니다. 실행 로그에서 원인을 확인하세요.</p>`
        : "";
      ...
      return `<article class="run-card">
        <div class="run-card-main"><div class="run-id">${escapeHtml(run.run_id)}</div><div class="run-meta"><span>${escapeHtml(run.preset || "-")}</span><span>${escapeHtml(run.mode || "-")}</span><span>${escapeHtml(statusText)}</span></div>${recoverable ? `<div class="run-reason">${canRecoverInput ? "입력을 복원해 새 분석으로 재시도할 수 있습니다." : "복원 가능한 입력이 없어 로그를 확인한 뒤 새 분석을 시작하세요."}</div>` : ""}${verificationNote}${outputsNote}</div>
  ...
  function renderDecision(summary) {
    ...
    const verification = state.detail?.run?.verification || null;
    const verified = verification?.checked === true && verification?.ok === true;
    ...
    const verificationNote = !verified
      ? `<div class="notice notice--warn">검증되지 않았거나 검증에 실패한 실행입니다. 이 판정을 근거로 인용하지 마세요.</div>`
      : "";
    const rerankPolicy = summary?.target_prediction?.rerank_policy;
    const rerankNote = rerankPolicy?.enabled === true
      ? `<div class="notice notice--warn"><strong>실험적 표적 재정렬이 활성화되었습니다.</strong> 현재 정책의 도킹 검증이 끝날 때까지 이 순위는 진단용이며 과학적 주장에 사용할 수 없습니다.</div>`
      : "";
    ...
    </div>${verificationNote}${rerankNote}<div class="next-action"><strong>다음에 할 일</strong><span>${escapeHtml(nextAction)}</span></div>`;
  }
  ...
  function efficacySummaryHtml(rows) {
    ...
    if (!tally.size) {
      return `<section class="surface"><div class="surface-heading"><div><p class="eyebrow">LITERATURE CO-OCCURRENCE</p><h2>피부 관련 문헌 공출현</h2></div></div><p class="muted">상위 표적과 피부 관련 키워드가 함께 나온 자동 문헌 연결이 없습니다. 이는 작용이나 효능의 유무를 판정한 결과가 아닙니다.</p></section>`;
    }
    const chips = Array.from(tally.entries())
      ...
      .join("");
    return `<section class="surface"><div class="surface-heading"><div><p class="eyebrow">LITERATURE CO-OCCURRENCE</p><h2>피부 관련 문헌 공출현</h2></div><span class="muted">자동 gene-keyword 공출현 · 검토 필요</span></div><div class="effect-chips">${chips}</div><p class="muted">유전자와 피부 관련 키워드가 같은 문헌에서 발견됐다는 뜻입니다. 작용 방향·기전·세포 또는 종 맥락은 검토되지 않았으며, <strong>이 화합물의 효능 근거가 아닙니다.</strong></p></section>`;
  }
  ...
  function renderResults() {
    ...
    const artifactRows = artifacts.map((artifact) => {
      ...
      const untrusted = artifact.verification_status === "untrusted_diagnostic";
      const citation = `${state.selectedRunId || "-"}:${artifact.path}`
        + (untrusted ? " · 검증 계약 위반 진단 파일(인용 금지)" : "");
      return `<div class="artifact-row"><span>${escapeHtml(artifact.label)}<small>${escapeHtml(citation)}</small></span><code>${escapeHtml(artifact.path)}</code><span class="artifact-actions"><span class="status-badge status-badge--${tone}">${escapeHtml(statusLabel(untrusted ? "untrusted_diagnostic" : status))}</span>${artifact.status === "available" && href ? `<a href="${escapeHtml(href)}" target="_blank" rel="noreferrer" title="새 탭에서 산출물 열기">열기</a>` : ""}</span></div>`;
    }).join("");
    ...
    content.innerHTML = `${substituteRun && substitutes?.status !== "blocked" ? renderSubstituteDecision(substitutes) : renderDecision(summary)}
      ...
      <details class="surface interpretation-guide">
        ...
        <dl class="glossary-list"><div><dt>PASS</dt><dd>필수 검증을 통과했다는 뜻입니다. 실제 효능이 입증됐다는 뜻은 아닙니다.</dd></div><div><dt>REVIEW</dt><dd>누락 근거나 피부 맥락을 사람이 확인해야 합니다. 내려받은 파일에는 <code>FLAG_HIGH</code>로 적혀 있는데 같은 판정입니다.</dd></div><div><dt>HALT</dt><dd>현재 결과를 효능이나 직접 결합 결론으로 사용하면 안 됩니다.</dd></div><div><dt>피부 반응 위험 점수</dt><dd>ADMET 모델의 피부 반응 예측값입니다. 판정과 함께 읽고 단독으로 안전성을 결론 내리지 마세요.</dd></div><div><dt>종합 예측 점수</dt><dd>같은 실행 안에서만 유효한 상대 순위입니다. 다른 실행과 절대값을 직접 비교하지 마세요. 승격된 레시피로 채점할 때는 <strong>근거 유사도와 다른 값</strong>입니다 — 가장 닮은 분자와의 거리뿐 아니라 그 측정이 얼마나 강했는지, 몇 개의 데이터베이스가 뒷받침하는지까지 함께 반영합니다. 순위를 재현하려면 이 열로 정렬하세요. 도킹·피부 점수는 순위에 반영되지 않는 주석입니다.</dd></div><div><dt>도킹 순위</dt><dd>여러 도킹·예측 방법이 매긴 순위를 하나로 합친 값입니다. 결합 실험값이 아닙니다. 유사도로만 순위를 매기는 기본 경로에서는 도킹이 순위에 반영되지 않아 종합 예측 점수와 같은 값이 됩니다.</dd></div><div><dt>조회 / 예측</dt><dd>“조회”는 입력 화합물과 지문이 같은 참조 리간드가 이 표적에서 이미 측정돼 있다는 뜻입니다. 즉 새로 예측한 것이 아니라 알려진 상호작용을 찾아온 것입니다. “예측”은 유사 화합물로부터 외삽한 결과입니다.</dd></div><div><dt>최근접 유사도</dt><dd>이 표적의 측정된 리간드 중 입력 화합물과 가장 가까운 것의 Tanimoto 값입니다. 1.0이면 조회입니다.</dd></div><div><dt>피부 점수</dt><dd>이 빌드의 단백질 사이에서 각 발현 축을 min-max 정규화한 탐색용 상대 피부 발현 맥락 점수입니다. <code>0.20</code>은 검토를 돕는 경험적 경계이며 효능 검증 기준이 아닙니다. 실제 기여 축은 <code>skin_score.tsv.axes.json</code>에서 확인하세요.</dd></div><div><dt>연결된 피부 문헌</dt><dd>유전자와 피부 관련 키워드의 자동 문헌 공출현 수입니다. 작용 방향·기전·세포·종 맥락은 사람이 검토해야 합니다. 일부 항목은 실제 집계가 아니라 큐레이션 seed 상수이며 (<code>n_papers_basis</code>로 구분), 검증된 PMID 수는 <code>n_papers_counted</code>입니다. 이 수를 효능이나 근거 강도로 읽지 마세요.</dd></div></dl>
      </details>
      <div class="result-grid">
        ...
        <div>
          <section class="surface result-section"><div class="surface-heading"><div><p class="eyebrow">SKIN CONTEXT</p><h2>피부 맥락</h2></div></div><div class="status-row"><span>상대 피부 발현 맥락 ≥ 0.20</span><span class="status-badge status-badge--${binding.skin_expression_supported ? "success" : "warning"}">${binding.skin_expression_supported ? "해당" : "미해당"}</span></div><div class="status-row"><span>피부 키워드 문헌 공출현</span><span class="status-badge status-badge--warning">${binding.skin_efficacy_requires_review === true || binding.skin_efficacy_supported === true ? "검토 필요" : "없음"}</span></div><div class="status-row"><span>상위 표적 맥락</span><span class="status-badge status-badge--warning">${binding.top_target_skin_context_supported ? "검토 필요" : "근거 부족"}</span></div><div class="status-row"><span>가장 피부 관련 표적</span><strong>${escapeHtml(binding.most_skin_relevant_target?.gene_symbol || binding.top_target_gene_symbol || "-")}</strong></div><p class="muted">발현 값과 문헌 공출현은 탐색용 맥락이며 효능을 입증하지 않습니다.</p></section>
  ...
  const THREE_D_LABEL = {
    ...
    partial_timeout: ["muted", "부분 평가 후 시간 상한",
      "일부 컨포머 조합만 평가한 뒤 시간 상한에 도달했습니다. 부분 최적값을 완료된 3D 판정으로 사용하지 않습니다."],
  ...
  function threeDCell(row) {
    ...
    if (status !== "ok") {
      ...
      const evaluated = Number.isInteger(row.three_d_evaluated_pairs)
        ? row.three_d_evaluated_pairs : null;
      const total = Number.isInteger(row.three_d_total_pairs)
        ? row.three_d_total_pairs : null;
      const coverage = evaluated !== null && total !== null && total > 0
        ? `<br><span class="muted">${escapeHtml(String(evaluated))}/${escapeHtml(String(total))} 조합 평가</span>`
        : "";
      return `<td><span class="${tone}" title="${escapeHtml(hint)}">${escapeHtml(label)}</span>${coverage}</td>`;
    }
  ...
  // 표적 검색: 단백질 → 그 표적에 측정 기록이 있는 화합물. CLI `explore_target`의
  // 서버 판이고 같은 생산 인덱스를 읽는다. 화면은 세 상태만 그린다 - 준비 안 됨,
  // 이름이 정확하지 않아 후보 목록, 결과 표.
  const TARGET_THRESHOLD = {
    positive: ["success", "양성"],
    borderline: ["warning", "경계"],
    below: ["danger", "문턱 아래"],
    unknown: ["neutral", "값 없음"],
  };
  const targetsState = { rows: [], uniprot: "", gene: "", description: "" };
  let targetsToken = null;

  function renderTargetsReadiness() {
    const panel = $("#targets-readiness");
    const run = $("#targets-run");
    if (!panel) return;
    const readiness = state.status?.analysis_readiness?.target_search;
    ...
    panel.innerHTML = "이 설치에는 표적 검색용 검색 인덱스가 없습니다. "
      + (missing.length
        ? `<strong>${missing.map(escapeHtml).join("</strong>, <strong>")}</strong>을 먼저 준비하세요.`
        : "Stage 0 표적 데이터를 먼저 준비하세요.");
    panel.classList.remove("is-hidden");
  ...
  function renderTargetCandidates(candidates) {
    ...
    if (!candidates || !candidates.length) {
      list.innerHTML = `<li class="name-result name-result--empty">이 이름으로는 표적을 찾지 못했습니다. UniProt 계정번호(예: P14679)를 직접 입력하세요.</li>`;
      return;
    }
    list.innerHTML = candidates.map((item) => `
      <li><button type="button" class="name-result" data-target="${escapeHtml(item.uniprot)}">
        <strong>${escapeHtml(item.gene || item.uniprot)}</strong>
        <code>${escapeHtml(item.uniprot)}</code>
        <small>${escapeHtml(item.description || "")}</small>
      </button></li>`).join("");
  }

  function targetThresholdLabel(key) {
    const entry = TARGET_THRESHOLD[key] || TARGET_THRESHOLD.unknown;
    return `<span class="status-badge status-badge--${entry[0]}">${entry[1]}</span>`;
  }

  function targetRowsHtml(rows) {
    if (!rows.length) {
      return `<tr><td colspan="8" class="muted">이 표적에 측정 기록이 있는 화합물이 인덱스에 없습니다. 표적이 4,873개 유니버스 밖이거나 측정이 아직 없을 수 있습니다.</td></tr>`;
    }
    return rows.map((row) => {
      const smiles = String(row.smiles || "");
      const shown = smiles.length > 48 ? `${smiles.slice(0, 48)}…` : smiles;
      const activity = row.max_pactivity === null || row.max_pactivity === undefined
        ? "-" : Number(row.max_pactivity).toFixed(2);
      return `<tr>
        <td>${formatCount(row.rank)}</td>
        <td><code title="${escapeHtml(row.inchikey || "")}">${escapeHtml(shown)}</code></td>
        <td class="score-cell">${activity}</td>
        <td>${targetThresholdLabel(row.threshold)}</td>
        <td>${formatCount(row.positive_measurement_count)}</td>
        <td>${formatCount(row.publication_count)}</td>
        <td>${escapeHtml(row.source_db || "-")}</td>
        <td><button type="button" class="button button--quiet button--small target-analyse" data-smiles="${escapeHtml(smiles)}" title="이 화합물을 새 분석 입력으로 보냅니다">이 후보 분석</button></td>
      </tr>`;
    }).join("");
  }

  function renderTargetResults(result) {
    const surface = $("#targets-surface");
    if (!surface) return;
    targetsState.rows = result.rows || [];
    targetsState.uniprot = result.target?.uniprot || "";
    targetsState.gene = result.target?.gene || "";
    targetsState.description = result.target?.description || "";
    const label = targetsState.gene
      ? `${targetsState.gene} (${targetsState.uniprot})` : targetsState.uniprot;
    $("#targets-target-note").innerHTML = `표적 <strong>${escapeHtml(label)}</strong>`
      + (targetsState.description ? ` — ${escapeHtml(targetsState.description)}` : "")
      + "에 측정 기록이 있는 화합물입니다. 순위는 유사도가 아니라 그 측정의 세기·양입니다.";
    $("#targets-count").textContent = `${formatCount(targetsState.rows.length)}개`;
    $("#targets-body").innerHTML = targetRowsHtml(targetsState.rows);
    surface.classList.remove("is-hidden");
  }

  async function runTargetSearch(queryOverride) {
    const input = $("#targets-query");
    const status = $("#targets-status");
    const badge = $("#targets-badge");
    const query = String(queryOverride ?? input?.value ?? "").trim();
    if (!query) {
      if (status) status.textContent = "표적 이름이나 UniProt 계정번호를 입력하세요.";
      return;
    }
    if (input) input.value = query;
    $("#targets-candidates")?.classList.add("is-hidden");
    $("#targets-surface")?.classList.add("is-hidden");
    const token = Symbol("targets");
    targetsToken = token;
    if (status) status.textContent = "검색 인덱스를 읽는 중…";
    if (badge) { badge.textContent = "조회 중"; badge.className = "status-badge status-badge--info"; }
    try {
      const result = await api("/api/target/binders", {
        method: "POST",
        body: JSON.stringify({
          target: query,
          mode: $("#targets-mode")?.value || "balanced",
          limit: Number($("#targets-limit")?.value || 20),
        }),
      });
      if (targetsToken !== token) return;
      if (result.available === false) {
        if (status) status.textContent = result.reason || "표적 검색을 실행할 수 없습니다.";
        if (badge) { badge.textContent = "준비 필요"; badge.className = "status-badge status-badge--warning"; }
        return;
      }
      if (result.resolved === false) {
        renderTargetCandidates(result.candidates || []);
        if (status) status.textContent = (result.candidates || []).length
          ? "정확히 일치하는 표적이 없습니다. 후보 중에서 고르세요."
          : (result.error || "표적을 찾지 못했습니다.");
        if (badge) { badge.textContent = "후보 확인"; badge.className = "status-badge status-badge--warning"; }
        return;
      }
      renderTargetResults(result);
      if (status) status.textContent = "";
      if (badge) { badge.textContent = "완료"; badge.className = "status-badge status-badge--success"; }
    } catch (error) {
      if (targetsToken !== token) return;
      if (status) status.textContent = error.message || String(error);
      if (badge) { badge.textContent = "오류"; badge.className = "status-badge status-badge--danger"; }
    }
  }

  // ---- 여러 표적 한 번에 발굴 (리스트 업로드/붙여넣기) ----
  // 표적 리스트는 서버로 보내 계산에만 쓰고 저장하지 않는다. 화면은 요약·표적별
  // 조회 상태·상위 후보 세 가지를 그린다.
  function parseTargetList(text) {
    ...
    const cells = (line) => line.split(delimiter).map((value) => value.trim());
    const headers = cells(lines[0]).map((value) => value.toLowerCase());
    const targetNames = ["target", "gene", "uniprot", "marker", "biomarker", "표적", "유전자"];
    const categoryNames = ["category", "카테고리", "분류", "group"];
    const targetIndex = headers.findIndex((value) => targetNames.includes(value));
    const categoryIndex = headers.findIndex((value) => categoryNames.includes(value));
    const hasHeader = targetIndex !== -1 || categoryIndex !== -1;
    const width = cells(lines[0]).length;
    const targetCol = targetIndex !== -1 ? targetIndex : 0;
    const categoryCol = categoryIndex !== -1 ? categoryIndex : width >= 2 ? 1 : -1;
    const body = hasHeader ? lines.slice(1) : lines;
    const rows = [];
    for (const line of body) {
      const values = cells(line);
      // 머리글에 category 열이 있어도, 한 칸짜리 줄은 그 칸을 표적으로 본다.
      // 이때 같은 칸을 카테고리로도 읽지 않는다.
      let label = String(values[targetCol] ?? "").trim();
      let category = categoryCol >= 0 && categoryCol !== targetCol
        ? String(values[categoryCol] ?? "").trim() : "";
      if (!label && values.length) {
        label = String(values[0] ?? "").trim();
        if (categoryCol === 0) category = "";
      }
      if (!label) continue;
      rows.push({ target: label, category });
    }
    return rows;
  }

  function targetBulkCoverageHtml(rows) {
    if (!rows.length) {
      return `<tr><td colspan="7" class="muted">조회 결과가 없습니다.</td></tr>`;
    }
    return rows.map((row) => `<tr>
      <td>${escapeHtml(row.category || "-")}</td>
      <td><code>${escapeHtml(row.target_label)}</code></td>
      <td>${escapeHtml(row.gene || "-")}</td>
      <td>${escapeHtml(row.uniprot || "-")}</td>
      <td>${row.in_index ? "가능" : '<span class="status-badge status-badge--warning">불가</span>'}</td>
      <td>${formatCount(row.n_candidates)}</td>
      <td class="muted">${escapeHtml(row.reason || "")}</td>
    </tr>`).join("");
  }

  function targetBulkCandidateHtml(rows) {
    if (!rows.length) {
      return `<tr><td colspan="9" class="muted">조회 가능한 표적이 없거나 후보가 없습니다. 표적별 조회 상태를 보세요.</td></tr>`;
    }
    return rows.map((row) => {
      const smiles = String(row.smiles || "");
      const shown = smiles.length > 40 ? `${smiles.slice(0, 40)}…` : smiles;
      const activity = row.max_pactivity === null || row.max_pactivity === undefined
        ? "-" : Number(row.max_pactivity).toFixed(2);
      const registered = row.cosing_match === "exact"
        ? `<span class="status-badge status-badge--success">동일</span>`
        : row.cosing_match === "connectivity"
          ? `<span class="status-badge status-badge--warning">연결성</span>`
          : row.cosing_match === "unavailable" ? "?" : "-";
      const inci = row.inci_name ? escapeHtml(row.inci_name) : "-";
      return `<tr>
        <td>${escapeHtml(row.category || "-")}</td>
        <td>${escapeHtml(row.gene || row.uniprot)}</td>
        <td>${formatCount(row.rank)}</td>
        <td><code title="${escapeHtml(row.inchikey || "")}">${escapeHtml(shown)}</code></td>
        <td class="score-cell">${activity}</td>
        <td>${targetThresholdLabel(row.threshold)}</td>
  ...
  async function runTargetListDiscovery() {
    const status = $("#targets-bulk-status");
    const rows = parseTargetList($("#targets-bulk")?.value || "");
    if (!rows.length) {
      if (status) status.textContent = "표적 행을 찾지 못했습니다. 한 줄에 하나씩 넣어주세요.";
      return;
    }
    if (status) status.textContent = "발굴 중…";
    $("#targets-bulk-surface")?.classList.add("is-hidden");
    try {
      const result = await api("/api/target/discover", {
        method: "POST",
        body: JSON.stringify({ rows, top: Number($("#targets-bulk-top")?.value || 10) }),
      });
      if (result.available === false) {
        if (status) status.textContent = result.reason || "발굴을 실행할 수 없습니다.";
        return;
      }
      if (result.error) {
      ...
      $("#targets-bulk-summary").innerHTML = `표적 <strong>${formatCount(summary.targets)}</strong> · `
        + `해석 <strong>${formatCount(summary.resolved)}</strong> · `
        + `조회 가능 <strong>${formatCount(summary.covered)}</strong> · `
        + `후보 <strong>${formatCount(summary.candidates)}</strong> · `
        + `등재 원료 일치 <strong>${formatCount(summary.registered)}</strong>`
        + (result.truncated ? " (후보 표는 300개까지만 보여줍니다)" : "")
        + `<br><span class="muted">${escapeHtml(result.measurement_note || "")}</span>`;
      $("#targets-bulk-count").textContent = `후보 ${formatCount((result.candidates || []).length)}행`;
      $("#targets-bulk-coverage-body").innerHTML = targetBulkCoverageHtml(result.coverage || []);
      $("#targets-bulk-candidates-body").innerHTML = targetBulkCandidateHtml(result.candidates || []);
      $("#targets-bulk-surface")?.classList.remove("is-hidden");
      if (status) status.textContent = "";
    } catch (error) {
      if (status) status.textContent = error.message || String(error);
    }
  ...
  function renderAxisWarning(ingredients) {
    ...
    if (!notes.length && rows && rows.total) {
      // 갖춰진 설치에서도 근거는 대부분 비어 있다 - 등재 원료 7,485종 중 활성
      // 측정 기록이 있는 것은 1,039종(13.9%)뿐이다. 그것은 결함이 아니라 사실이고,
      ...
      if (rows.evidence < rows.total) {
        notes.push(`이 표 ${rows.total}건 중 활성 측정 기록이 있는 것은 `
          ...
          + `채워집니다 - 등재 원료 전체에서도 기록이 있는 것은 13.9%입니다.`);
      }
    }
  ...
  function renderSortNote(sortBy, pharmacophoreUnavailable) {
    ...
    panel.textContent =
      ...
      + "지금 이 화면이 훑는 것은 7,485종입니다 - 모집단이 15배 커져 재평가에 며칠이 "
  ...
  // 얼마나 걸릴지 미리 말해 준다. 이 화면은 라이브러리 7,485종 전부에 최대공통
  ...
  function alternativesCsv() {
    ...
    ["structure_rank", (r) => r.original_rank, () => ""],
      ...
      ["three_d_evaluated_pairs", (r) => r.three_d_evaluated_pairs ?? "", () => ""],
      ["three_d_total_pairs", (r) => r.three_d_total_pairs ?? "", () => ""],
  ...
  function bindEvents() {
    document.addEventListener("click", async (event) => {
      ...
      const targetAnalyse = event.target.closest(".target-analyse[data-smiles]");
      if (targetAnalyse) {
        // 대체소재의 '이 후보 분석'과 같은 점프. 표적 검색에서 고른 화합물이
        // 새 분석 입력으로 넘어가야 조회한 표적과 실제 실행을 견줄 수 있다.
        setInputType("smiles");
        const input = $("#smiles-input");
        if (input) {
          input.value = targetAnalyse.dataset.smiles;
          input.dispatchEvent(new Event("input", { bubbles: true }));
        }
        navigate("analyze");
        showToast("화합물을 새 분석 입력에 넣었습니다.");
        return;
      }
    ...
    $("#targets-run")?.addEventListener("click", () => runTargetSearch());
    $("#targets-query")?.addEventListener("keydown", (event) => {
      if (event.key === "Enter") { event.preventDefault(); runTargetSearch(); }
    });
    $("#targets-candidates")?.addEventListener("click", (event) => {
      const button = event.target.closest(".name-result[data-target]");
      if (!button) return;
      $("#targets-candidates")?.classList.add("is-hidden");
      runTargetSearch(button.dataset.target);
    });
    $("#targets-bulk-run")?.addEventListener("click", () => runTargetListDiscovery());
    $("#targets-bulk-file")?.addEventListener("change", (event) => {
      const file = event.target.files?.[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => {
        const area = $("#targets-bulk");
