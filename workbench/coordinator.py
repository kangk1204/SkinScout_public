...
class DurableCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        if state_dir != Path("."):
            self._ensure_private_directory(state_dir)
        self.artifact_root = (
            Path(artifact_root)
        ...
        self._validate_private_ancestors(state_dir)
        self._validate_private_ancestors(self.artifact_root)
        self._clock = clock
    ...
    def promote_attempt_artifacts(
        ...
        *,
        ...
        content_sha256 = hashlib.sha256(manifest_text.encode("utf-8")).hexdigest()
        namespace_path = self._safe_relative_path(namespace, "artifact namespace")

        # ``content_sha256`` identifies the immutable bytes. ``artifact_id``
        # identifies this job/attempt's registration of those bytes.  Using the
        # content hash for both made two independent jobs that produced the same
        # valid output collide in the global registry.
        ...
        artifact_id = hashlib.sha256(association.encode("utf-8")).hexdigest()
        ...
        published = False
        try:
            self._copy_verified_workspace(workspace, staging, canonical_manifest)
    ...
    def _copy_verified_workspace(
        self,
        workspace: Path,
        staging: Path,
        manifest: dict[str, Any],
    ) -> None:
        """Copy manifest files into a fresh staging inode before sealing.

        The worker created the workspace files, so a retained writable file
        descriptor could still mutate them after a rename-based promotion. The
        sealed copy is written to new inodes the worker never held open.
        """
        ...
        try:
            ...
            raise CoordinatorError("artifact staging path already exists") from exc
        staging.chmod(0o700)
    ...
    def _copy_verified_file(
        ...
        try:
            ...
            raise InvalidTransition(
                f"attempt workspace file is missing or unsafe: {source_path.name}"
            ) from exc
        try:
            if not stat.S_ISREG(os.fstat(source_fd).st_mode):
                raise InvalidTransition(
                    f"attempt workspace file is not a regular file: {source_path.name}"
                )
        ...
        if written != entry["bytes"] or digest.hexdigest() != entry["sha256"]:
            raise InvalidTransition(f"artifact bytes mismatch: {entry['path']}")

    @staticmethod
    def _ensure_private_directory(path: Path) -> None:
        if not path.name:
            raise CoordinatorError(f"refusing to restrict an unsafe directory: {path}")
        try:
        ...
        if stat.S_IMODE(path.lstat().st_mode) != 0o700:
            raise CoordinatorError(f"artifact directory is not private: {path}")

    @staticmethod
    def _validate_private_ancestors(path: Path) -> None:
        group_reachable = True
        other_reachable = True
        for ancestor in list(path.absolute().parents)[::-1]:
            try:
                info = ancestor.stat()
            except OSError as exc:
                raise CoordinatorError(
                    f"private state ancestor is not accessible: {ancestor}"
                ) from exc
            if not stat.S_ISDIR(info.st_mode):
                raise CoordinatorError(
                    f"private state ancestor is not a directory: {ancestor}"
                )
            group_reachable = group_reachable and bool(info.st_mode & 0o010)
            other_reachable = other_reachable and bool(info.st_mode & 0o001)
            if info.st_mode & stat.S_ISVTX:
                continue
            if group_reachable and info.st_mode & 0o020:
                raise CoordinatorError(
                    f"state directory ancestor is writable by others: {ancestor}"
                )
            if other_reachable and info.st_mode & 0o002:
                raise CoordinatorError(
                    f"state directory ancestor is writable by others: {ancestor}"
                )
    ...
    def _ensure_private_database_file(self) -> None:
        ...
        try:
            ...
            raise CoordinatorError(
                f"cannot open coordinator database safely: {self.db_path}"
            ) from exc
        os.close(descriptor)
        for suffix in ("", "-wal", "-shm", "-journal"):
            candidate = Path(str(self.db_path) + suffix)
            if suffix and not candidate.exists():
                continue
            try:
                candidate.chmod(0o600)
            except OSError as exc:
                raise CoordinatorError(
                    f"cannot restrict coordinator database permissions: {candidate}"
                ) from exc
            if stat.S_IMODE(candidate.stat().st_mode) != 0o600:
                raise CoordinatorError(
                    f"coordinator database file is not private: {candidate}"
                )
