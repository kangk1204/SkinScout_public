...
_FAILURE_MODE_CACHE: dict[str, dict[str, str]] | None = None
_FAILURE_MODE_CACHE_LOCK = threading.Lock()


def _failure_mode_records() -> dict[str, dict[str, str]]:
    """Curated receptor limitations, read once and reused across requests."""
    global _FAILURE_MODE_CACHE
    if _FAILURE_MODE_CACHE is not None:
        return _FAILURE_MODE_CACHE
    with _FAILURE_MODE_CACHE_LOCK:
        if _FAILURE_MODE_CACHE is not None:
            return _FAILURE_MODE_CACHE
        try:
            _FAILURE_MODE_CACHE = load_curated(ROOT / "data" / "validation" / "known_failure_modes.csv")
        except SystemExit:
            # The browser view degrades to no warnings rather than refusing to
            # render a run the pipeline already produced.
            _FAILURE_MODE_CACHE = {}
    return _FAILURE_MODE_CACHE
...
def _summary_card(
    ...
    *,
    ...
    return {
        ...
        # A claim needs both the pipeline's own verdict and a passing verifier.
        "claimable": overall.get("claimable") is True and verification["ok"],
...
def _verification_state(run_id: str) -> dict[str, Any]:
    ...
    return {
        "checked": True,
        "ok": status == "ok" and verifier_status == "ok",
        "status": status if isinstance(status, str) else None,
        "verifier_status": verifier_status if isinstance(verifier_status, str) else None,
        "error": (
            payload.get("verifier_contract_error")
            if isinstance(payload.get("verifier_contract_error"), str)
            else None
        ),
    }
...
def list_runs() -> list[dict[str, Any]]:
    ...
    for directory in directories[:100]:
        ...
        if summary is None and active is None:
            continue
        rows.append(
            _summary_card(
                run_id,
                summary,
                active.status if active else "completed",
                analysis_kind=_analysis_kind(durable_by_run.get(run_id)),
            )
        )
...
def _run_file_for_download(run_id: str, relative: str) -> Path | None:
...
def _run_bundle_zip(run_id: str) -> bytes | None:
    ...
    for name in BUNDLE_ROOT_FILES:
        resolved = _run_file_for_download(safe_run_id, name)
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        for path, relative in members:
            archive.write(path, arcname=f"{safe_run_id}/{relative}")
...
def _target_rows(
    ...
    *,
    ...
    if path is None:
        ...
        return {"rows": rows[:limit], "total": len(rows), "source_path": None, "exploratory": True}
    ...
    try:
        ...
        return {"rows": [], "total": 0, "source_path": _relative_path(path), "exploratory": True, "error": "ranking 파일을 읽지 못했습니다."}
    if sort == "skin_score":
...
def _start_run(payload: dict[str, Any]) -> Job:
    ...
    if input_type == "smiles":
        ...
        # The upload is on disk before it can be judged, so it has to be removed
        # on every path that does not start a run - including one where the
        # check itself raises.
        try:
...
class WorkbenchHandler(BaseHTTPRequestHandler):
    ...
    def _request_host_is_loopback(self) -> bool:
        parsed = urlsplit("//" + self.headers.get("Host", ""))
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/identity":
            self._send_json({"application": "SkinScout Workbench"})
            return
        if path == "/api/status":
    ...
    def do_POST(self) -> None:  # noqa: N802
        path = urlsplit(self.path).path
        # A remote bind is only permitted with authentication on, so the
        # loopback rule is the right one whenever auth is off.
        if not REQUIRE_AUTH and not self._request_host_is_loopback():
            self._error("상태 변경은 이 컴퓨터의 로컬 주소에서만 허용됩니다.", 403)
            return
        if path == "/api/session":
...
def create_server(
    ...
    *,
    ...
    A non-loopback bind used to be refused outright, which left an SSH tunnel
    as the only way to reach a lab GPU box. It is allowed now, but only with
    authentication on: every read endpoint was open, so exposing the port
    without a login handed anyone on the network the results and the ability to
    start runs.
    """
    global REQUIRE_AUTH
    remote = host not in LOOPBACK_HOSTS
    REQUIRE_AUTH = remote if require_auth is None else bool(require_auth)
...
def main(argv: list[str] | None = None) -> int:
    ...
    server = create_server(
        args.host, args.port, require_auth=True if args.require_auth else None
    )
